...
[적대자 위협 실체]
- 적대자의 구체적 목표: ___
- 적대자가 성공하면 파괴되는 것: ___ ("위험해진다" 같은 추상어 금지. 구체적으로.)
- 적대자의 타이머/데드라인: ___ (시간 압박이 있는가? 없으면 만들어라.)
...
0-1. 이 비트를 집필하기 전에 아래 5개 질문에 먼저 답하라. 답이 씬에 반영되어야 한다.
  ...
  ③ 적대자 행동: 이 비트에서 적대자(또는 적대 세력)가 구체적으로 무엇을 했는가?
     → "위협적이다"가 아니라 "무엇을 했다"로 써라. 행동이 없으면 적대자가 죽어 있다.
     → 적대자가 이 비트에 직접 등장하지 않더라도, 그의 영향이 느껴져야 한다.
