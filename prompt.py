# ─────────────────────────────────────────────────────────────
# BLUE JEANS SCREENPLAY WRITER ENGINE v3.7.2
# prompt.py — Full Version (Creator Engine v2.5.5 동기화)
# © 2026 BLUE JEANS PICTURES
#
# v3.7.2 주요 변경사항 (2026-05-20):
# - 상업 장르 영화 1씬 대사 의무 룰 — 전 장르로 확대 적용 (Mr. MOON 통찰)
#   * 진단: v3.7.1은 호러 장르에만 1씬 대사 의무 적용.
#     "상업 장르 영화 전체에 적용해야 한다"는 Mr. MOON님 통찰을 받아
#     OPENING MASTERY 모듈 레벨에서 절대 룰로 격상.
#   * 적용 장르: 호러, 액션, 스릴러, 범죄, 누아르, 코미디, 로맨스,
#     멜로, 로맨틱 코미디, SF, 판타지, 시대극 액션/스릴러
#   * 예외: 정통 드라마만 침묵 도입 허용 (조건부 — 침묵 후 3분 안에
#     결정적 대사 1줄 등장 필수)
#   * OPENING MASTERY에 "상업 장르 영화 1씬 대사 절대 룰" 신규 블록 신설
#     - 장르 영화 vs 아트하우스 영화의 차이 명시
#     - 무성영화 테스트 자가 검토
#     - 장르별 1씬 대사 형식 권장 (8개 장르 각각 레퍼런스 포함)
#   * 각 장르 항목에도 "★ v3.7.2: 1씬 대사 1줄 이상 필수" 표시 추가
#   * "설명적 오프닝 금지 패턴"에 "대사 0줄" 항목 추가
#
# v3.7.1 주요 변경사항 (2026-05-20):
# - 호러 1씬 장르 영화 대사 의무 룰 추가 — Mr. MOON 통찰 반영
#   * 진단: v3.7.0의 호러 1씬이 대사 없는 아트하우스 호러 문법으로 출력됨.
#     "장르 영화 1씬은 대사로 시작해야 한다. 대사 없는 1씬은 아트하우스 영화."
#     상업 호러(컨저링·인시디어스·스크림 계열)와 아트하우스 호러(유전·바바둑)
#     구분이 룰에 명시되지 않아 발생한 누락.
#   * HORROR DNA #6에 1씬 대사 4유형 추가:
#     - Cold Open Dialogue (안전지대 대화): 〈스크림〉, 〈인시디어스〉
#     - Mystery Hook Dialogue (톤 선언): 〈유전〉, 〈식스 센스〉
#     - Phone Call / Communication: 〈스크림〉 전화, 〈겟 아웃〉 통화
#     - Witness Dialogue (목격자 확인 대사)
#   * 대사 + 마무리 조합 최강 패턴 4종 명시
#   * 자가 검토 3문항 (무성영화 테스트 / 동일시 모드 / 대사 필수성)
#   * 자동 위반 룰: 1씬 대사 0줄 = 무조건 재집필
#
# - OPENING MASTERY 호러 섹션에도 대사 1줄 이상 필수 추가
#   * 레퍼런스 영화 5편 인용 (스크림·컨저링·겟 아웃·유전·인시디어스)
#
# v3.7.0 주요 변경사항 (2026-05-20):
# - 호러 1씬 결정적 마무리 룰 추가 — Mr. MOON 통찰 반영
#   * 진단: 바타비아 호러 1씬이 분위기만 누적되고 끝남.
#     "공포감이 시작된다면 누군가 비명으로 끝나야 한다"는 호러 작법의 본질.
#   * GENRE_BOOSTER_HORROR에 DNA #6 추가 (5종 → 6종)
#   * Beat 1 첫 씬(S#1)에만 적용되는 절대 규칙
#   * 1씬 마무리 4유형 강제: 비명 / 시체·흔적 / 실체 1초 노출 / 신체 반응
#   * 참조: 〈유전〉 비둘기 시체 / 〈컨저링〉 어머니 비명 / 〈그것〉 페니와이즈 노출
#   * 1씬 자가 검증 게이트 신설 (다른 비트 룰 통과해도 이 룰만은 통과 필수)
#
# - A28. 씬 번호 연속성 룰 신설
#   * 진단: 바타비아 호러 Beat 1에서 S#1 → S#3로 점프, S#2 누락 발생.
#     AI가 비트별 집필 시 씬 번호 카운팅을 건너뛰는 버그.
#   * 룰: 씬 번호는 1, 2, 3, 4 순서대로 빈틈없이 증가
#   * 비트 간 연속성: 직전 비트 마지막 씬 번호 + 1부터 시작
#   * 비트 종료 시 자가 검증 게이트
#
# - A29. 지문 단락 과잉 분단 방지 룰 신설
#   * 진단: 바타비아 호러 1씬이 12개 단락으로 잘게 쪼개짐.
#     A22(한 문장=한 샷)를 과해석해 카메라 컷 단위로 끊은 결과.
#     prompt.py "한 단락 2~4줄" 룰을 위반.
#   * 룰: 카메라 컷이 아니라 의미 비트 단위로 단락 분리
#   * 한 씬 지문 단락 8개 이하 / 1문장 단락 5개 미만 강제
#   * 1문장 단독 단락 예외: 결정적 강조 / 시간 점프 / 인물 등장만 허용
#   * A22와의 관계 명시: A22 = 문장 단위 / A29 = 단락 단위 (함께 작동)
#
# - GENRE_BOOSTER_HORROR 버전 v3.2 → v3.7
# - ENGINE_BUILD_DATE 2026-05-16 → 2026-05-20
#
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━
GENRE BOOSTER — HORROR (호러 비트 강제 규칙 v3.7)
━━━━━━━━━━━━━━━━━━━━━━━━━━━

★ 매 비트 집필 시 다음 5가지 중 최소 3개를 비트 안에 강제 배치하라.
★ Beat 1(Opening Image)에 한해 6번 룰이 추가로 강제된다.
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━
[★★★ HORROR DNA #6 — 첫 씬 결정적 마무리 (v3.7 신규) ★★★]
━━━━━━━━━━━━━━━━━━━━━━━━━━━

★ 이 룰은 Beat 1(Opening Image)의 첫 씬(S#1)에만 적용되는 절대 규칙이다.
★ DNA 1~5의 분위기 누적만으로 1씬을 끝내지 마라. 호러는 관객과의 첫 약속이고,
   그 약속은 분위기가 아니라 결정적 한 방으로 체결된다.

[근거 — 시나리오 작법]
- 호러 1씬은 관객에게 "이 영화는 무섭다"는 계약서다.
- 누적만 있고 결론이 없으면 관객은 "분위기 영화"로 오해한다.
- 〈유전〉 1씬: 미니어처 → 부고 → 어머니 헌사 → 비둘기 시체 (결정적 한 방 = 비둘기 시체)
- 〈컨저링〉 1씬: 인형 애나벨 + 어머니의 비명
- 〈그것〉 1씬: 페니와이즈 + 조지의 절단 비명
- 〈인시디어스〉 1씬: 검은 사람 + 어머니의 비명

[1씬 마무리 4유형 — 반드시 하나 선택]

유형 A. 비명 (Scream)
   - 누군가의 비명으로 1씬을 끝낸다.
   - 화면 안 비명 / 화면 밖 비명 / 벽 너머 비명 / 꿈속 비명 / 전화기 너머 비명
   - 비명의 주인은 명시하지 않아도 된다 (Mystery Box 유지)
   - 1씬 마지막 지문이 비명 자체이거나, 비명을 듣고 정지한 인물

유형 B. 시체 또는 결정적 흔적
   - 1씬 마지막에 시체, 핏자국, 파괴된 사물, 흩어진 물건의 발견
   - "이미 무언가 일어났다"의 증거
   - 추상적 흔적 금지, 시각적으로 확인 가능한 구체물

유형 C. 위협의 실체 1초 노출 (Glimpse)
   - 거울·창·그림자·물 표면에 위협의 형태가 1초 또는 그보다 짧게 노출
   - 인물은 못 봤거나, 보고 자기 눈을 의심하는 순간
   - 관객만 보고, 다음 컷에는 사라진다
   - 노출은 짧을수록 강하다 (보는 것보다 보일까 말까가 무섭다)

유형 D. 인물의 신체 반응 — 비명에 준하는 결정적 반응
   - 비명을 지르려다 막힘, 호흡 정지, 토함, 도주, 기절, 무너짐
   - 분석 행동(노트 펴기, 책 찾기 등) 금지
   - 인물이 "이건 보통이 아니다"를 신체로 선언해야 한다

[★ 1씬 자가 검증 — 위 4유형 중 반드시 1개 이상 충족]

  □ 1씬 마지막에 비명·시체·실체 노출·신체 반응 중 하나가 있는가?
  □ 1씬을 본 관객이 "이 영화는 무섭다"를 알 수 있는가?
  □ Maya가 노트를 펴고 분석하는 식의 미스터리 주인공 행동으로 끝나지는 않는가?
  → 미달 시 1씬 재작성. 다른 비트 룰은 통과해도 이 룰만은 통과 필수.

[금지 — 호러 1씬 실패 패턴]
❌ 누적된 이상 신호만 있고 결정적 한 방 없이 다음 씬으로 넘김
❌ 주인공이 분위기를 분석·연구·기록하는 행동으로 1씬 마무리
❌ 주인공이 "잠을 못 잤다"로 1씬 끝 (생리적 반응이지 공포 반응 아님)
❌ 1씬 분량의 70% 이상이 환경 묘사 (위협 < 환경)

[1씬 비트 구조 권장]
- 도입: 감각 도입 (소리 또는 본질적으로 불편한 첫 이미지)
- 전개: 흔적 누적 (DNA 1~5 적용, 3겹 이상)
- 클라이맥스: ★ 4유형 중 하나 강제 ★
- 마무리: 결정적 한 방 후 1~2줄로 종료 (긴 후일담 금지)

━━━━━━━━━━━━━━━━━━━━━━━━━━━
[★★★ DNA #6 보강 — 장르 영화 1씬 대사 의무 (v3.7.1 신규) ★★★]
━━━━━━━━━━━━━━━━━━━━━━━━━━━

★ 호러 1씬은 반드시 대사 1줄 이상 포함해야 한다.
★ 대사 없는 1씬은 아트하우스 호러(타르콥스키·베리만 계열) 문법이다.
★ 상업 호러는 첫 대사로 관객-인물 동일시를 시작한다.

[근거 — 장르 영화 vs 아트하우스 영화]
- 장르 영화: 첫 1분 안에 관객 동일시 발생 필수. 대사 = 동일시의 입구.
- 아트하우스: 관객은 관찰자로 남는다. 대사 없이 명상.
- 호러는 상업 장르다. 관객이 인물을 무서워하지 않으면 영화가 안 무섭다.
- 관객이 인물에 동일시하지 않으면 무서워하지 않는다.
- 대사 없는 첫 씬 = 관찰자 모드 = 호러 실패.

[1씬 대사 형식 4유형 — 반드시 하나 선택]

대사형 1. Cold Open Dialogue — 위협 직전의 일상 대화
   - 인물들이 평범한 대화를 나누는 중에 위협이 시작
   - 안전지대 셋업 → 균열 → 침범의 고전 구조
   - 레퍼런스: 〈스크림〉 케이시 + 전화 / 〈겟 아웃〉 길 잃은 남자 통화 /
     〈인시디어스〉 부부의 새 집 입주 대화

대사형 2. Mystery Hook Dialogue — 미스터리 톤 선언 대사
   - 한 줄로 작품의 톤과 미스터리 코어를 선언
   - 누구의 대사인지 명시하지 않아도 된다 (보이스오버 가능)
   - 레퍼런스: 〈유전〉 어머니의 헌사 ("My mother was a secretive woman...")
     / 〈식스 센스〉 의사의 자축 대화

대사형 3. Phone Call / Communication — 통신 매개 대사
   - 전화·라디오·인터컴·텔레비전을 통한 대사
   - 화자의 얼굴이 안 보여도 작동
   - 거리감과 위협을 동시에 만든다
   - 레퍼런스: 〈스크림〉 전화 / 〈웬 어 스트레인저 콜스〉 / 〈겟 아웃〉 통화

대사형 4. Witness Dialogue — 목격자의 첫 대사
   - 위협을 목격한 인물의 짧은 대사 (한 단어도 가능)
   - "Did you see that?" "뭐였어 지금?" "들리지?" 같은 확인 대사
   - 관객도 같은 의문을 갖게 만드는 동일시 장치

[★ 1씬 대사 자가 검증]

  □ 1씬에 대사가 1줄 이상 있는가?
  □ 그 대사가 위 4유형 중 하나에 해당하는가?
  □ 대사 없이 환경 묘사 + 인물 행동만으로 1씬이 구성되지 않았는가?
  □ 대사가 작품의 톤·미스터리·관계를 한 줄로 선언하는가?
  → 1씬 대사 부재는 자동 위반. 4유형 중 하나로 재집필.

[금지 — 호러 1씬 대사 실패 패턴]
❌ 1씬 전체에 대사 0줄 (아트하우스 호러 문법)
❌ 인물이 혼자 등장해 침묵으로만 1씬 진행
❌ 대사가 있어도 의미 없는 일상 단어("...", "어?", "뭐야?") 단발성으로만 사용
❌ 보이스오버 내레이션만 있고 캐릭터 간 대사 부재

[장르 영화 1씬 대사 + DNA #6 결정적 마무리 조합 — 최강 패턴]

대사형 1 (안전지대 대화) + 유형 A (비명):
  → 〈스크림〉 패턴. 가장 효과적인 호러 1씬 구조.
  → 일상 대화 → 균열 → 비명으로 마무리.

대사형 2 (미스터리 톤 대사) + 유형 B (시체·흔적):
  → 〈유전〉 패턴. 톤 선언 후 결정적 시각 한 방.

대사형 3 (전화/통신) + 유형 C (실체 1초):
  → 〈겟 아웃〉 패턴. 거리감 위협 + 짧은 노출.

대사형 4 (목격자 대사) + 유형 D (신체 반응):
  → 〈인시디어스〉 패턴. 확인 대사 + 결정적 반응.

[★ 작가 자가 검토 — 1씬 완성 후 마지막 점검]

  Q1. 이 1씬을 무성영화로 만들면 같은 효과가 나는가?
      → YES면 아트하우스 호러. NO면 장르 호러 (정답).
  Q2. 이 1씬을 본 관객이 인물의 입장에서 무서워하는가, 인물을 관찰하며
      무서워하는가?
      → 관찰자 모드면 동일시 실패. 입장에서 무서워해야 정답.
  Q3. 대사 1줄을 빼도 1씬이 작동하는가?
      → YES면 그 대사는 장식. 작동에 필수인 대사를 써라.
""".strip()
...
[A28. 씬 번호 연속성 — Scene Numbering Continuity (v3.7 신규)]
이 룰은 비트별 집필 시 AI가 씬 번호 카운팅을 건너뛰는 버그를 차단한다.

❌ 발견된 실패 사례 (바타비아 호러 Beat 1):
S#1. INT. VOC 저택 신혼 침실 — 새벽 4시
S#3. INT. VOC 저택 1층 식당 — 아침      ← S#2가 누락
S#4. EXT. VOC 저택 현관 담장 — 아침

✅ 올바른 출력:
S#1 → S#2 → S#3 → S#4 → ...

원칙 — 비트 집필 시 씬 번호 규칙:
1. **연속성** — 씬 번호는 1, 2, 3, 4... 순서대로 빈틈없이 증가한다.
   ❌ S#1 → S#3 (S#2 누락)
   ❌ S#1 → S#1 (중복)
   ❌ S#2 → S#1 (역행)
   ✅ S#1 → S#2 → S#3

2. **비트 간 연속성** — 한 비트의 첫 씬 번호는 직전 비트의 마지막 씬 번호 + 1이다.
   Beat 1 마지막 씬이 S#5이면, Beat 2 첫 씬은 S#6이어야 한다.

3. **비트 내 연속성** — 한 비트 안에서 여러 씬을 쓸 때 번호는 무조건 연속.
   비트 안에서 씬 순서를 바꿔도 출력 순서는 번호 순.

4. **재집필 시 보존** — 특정 씬을 재집필할 때 번호는 그대로 유지.
   기존 S#3을 다시 쓰면 결과도 S#3이어야 한다.

[★ 씬 번호 자가 검증 — 비트 종료 시 점검]
  □ 씬 번호가 1씩 증가하는가?
  □ 직전 비트 마지막 씬 번호 + 1부터 시작하는가?
  □ 같은 번호가 두 번 나오지 않는가?
  → 미달 시 씬 번호 재정렬 후 출력.

[A29. 지문 단락 과잉 분단 방지 — Anti Over-Fragmentation (v3.7 신규)]
이 룰은 A22(한 문장=한 샷)를 과해석한 결과 1씬이 12개 단락으로 쪼개지는
버그를 차단한다. A22는 의미 단위 룰이지 단락 분리 룰이 아니다.

❌ 발견된 실패 사례 (바타비아 호러 S#1, v3.6.1 출력):
[단락 1] 암전.
[단락 2] 소리만 있다. 벽체를 타고 전해지는 저주파 진동. (1문장 덩어리)
[단락 3] 카메라가 천천히 노출을 열기 시작하면 흰 벽면이 프레임을 채운다. (1문장 덩어리)
[단락 4] 벽면 익스트림 클로즈업. 피의 인장. (1문장 덩어리)
... 총 12개 단락. 각 단락이 카메라 컷 단위로 끊김.
→ 한 씬의 호흡이 무너짐. 호러의 정적 압박이 오히려 약해짐.

✅ 의미 비트 단위로 묶은 출력 (같은 내용, 4개 단락):
[비트 A — 청각 도입 + 봉인 발견]
암전. 소리만 있다... 카메라가 천천히 노출을 열면... 벽면 클로즈업, 피의 인장... 자장가가 끊긴다.
[비트 B — 벽이 Maya를 인식]
Maya가 손을 뗀다. 벽면에 손바닥 모양의 열기 자국이 남았다 사라진다.
[비트 C — 공간 와이드 + Reza의 잠]
침실 전체가 보인다... Reza가 잠들어 있다.
[비트 D — Maya의 발견과 외면]
Maya가 노트를 꺼낸다... 노트를 덮는다. 잠들지 않는다.

원칙 — 지문 단락 분리 시 자가 점검:
1. **카메라 컷 단위로 끊지 마라. 의미 비트 단위로 끊어라.**
   - 카메라 컷: 한 샷 = 한 단락 (✗ 콘티 문법)
   - 의미 비트: 한 흐름 = 한 단락 (✓ 시나리오 문법)

2. **최소 단락 길이** — 1문장만으로 단락을 끝내지 마라. 단, 다음 경우만 예외:
   ① 결정적 단독 강조 ("암전.", "침묵.", "같은 창. 같은 자리. 150년.")
   ② 시간 점프 단락 ("3개월 후.")
   ③ 인물 등장 선언 단락 ("Reza가 들어온다.")

3. **씬당 1문장 단락 상한** — 한 씬 안에서 1문장 단락이 5개 이상이면 자동 위반.
   의미 비트 단위로 재병합한다.

4. **씬당 총 단락 수 상한** — 한 씬의 지문 단락이 8개 이상이면 의미 비트 검토.
   - 8개 이상 = 카메라 컷 단위로 끊긴 신호
   - 액션 씬 / 몽타주 씬은 예외 가능 (검토 후 유지)

5. **호러 장르 예외** — 정적 압박을 위한 짧은 단락은 결정적 한 줄("암전.",
   "침묵.")에만 허용. 환경 묘사·동작 묘사는 의미 비트로 묶는다.

[★ 단락 자가 검증 — 씬 종료 시 점검]
  □ 한 씬 지문 단락이 8개 이하인가?
  □ 1문장 단락이 5개 미만인가?
  □ 1문장 단락이 결정적 강조 / 시간 점프 / 인물 등장 중 하나에 해당하는가?
  □ 의미 비트가 같은 단락들이 함께 묶여 있는가?
  → 미달 시 재병합.

[A22와의 관계]
A22 "한 문장 = 한 샷"은 의미 단위 룰이지 단락 분리 룰이 아니다.
A22로 문장 단위를 끊되, A29로 끊은 문장들을 의미 비트 단위로 한 단락에 묶는다.
두 룰은 함께 작동한다 — 문장은 A22로 끊고, 단락은 A29로 묶는다.
...
▸ 호러 / 공포 → 권장 기법: Tease & Reveal / Cold Open
   ...
   - ★ v3.7.1: 첫 씬에 대사 1줄 이상 필수. 대사 없는 1씬은 아트하우스 호러 문법.
     상업 호러는 첫 대사로 관객-인물 동일시를 시작한다.
     레퍼런스: 〈스크림〉 "What's your favorite scary movie?" / 〈컨저링〉 인터뷰 /
     〈겟 아웃〉 길 잃은 남자의 통화 / 〈유전〉 어머니의 헌사 / 〈인시디어스〉 부부 대화

━━━━━━━━━━━━━━━━━━━━━━━━━━━
[★★★ 상업 장르 영화 1씬 대사 절대 룰 (v3.7.2 신규) ★★★]
━━━━━━━━━━━━━━━━━━━━━━━━━━━

★ 이 룰은 호러를 포함한 모든 상업 장르 영화에 적용되는 절대 규칙이다.
★ 적용 장르: 호러 / 액션 / 스릴러 / 범죄 / 누아르 / 코미디 / 로맨스 / 멜로 /
            로맨틱 코미디 / SF / 판타지 / 시대극 액션 / 시대극 스릴러
★ 제외: 정통 드라마 (침묵 도입 허용, 다만 권장은 함)

[근거 — 장르 영화 vs 아트하우스 영화의 결정적 차이]

장르 영화 = 관객 동일시의 문법:
- 첫 1분 안에 관객은 누군가의 입장에서 듣는다 (대사 = 동일시 입구)
- 관객이 인물 안으로 들어가지 못하면 장르적 쾌감(공포·웃음·서스펜스·로맨스)
  자체가 발생하지 않는다
- 호러는 인물이 무서워해야 관객이 무섭다 — 무서워하는 인물을 들어야 한다
- 코미디는 인물의 코믹 보이스를 들어야 한다 — 첫 대사가 첫 웃음
- 로맨스는 인물의 사랑에 대한 태도를 들어야 한다 — 대사 톤이 모든 것
- 스릴러는 정보 비대칭을 대사로 깐다 — 누가 무엇을 아는지 대사로 선언
- 액션은 액션 중에도 대사가 있다 — 〈매드맥스〉 분노의 도로조차 첫 1분에 대사

아트하우스 영화 = 관찰자 모드의 문법:
- 관객은 명상적 거리에서 인물을 응시한다
- 대사 없는 침묵 도입은 의도적 장치
- 타르콥스키, 베리만, 아피찻퐁, 차이밍량의 문법
- 상업 영화의 문법이 아니다

[1씬 대사 의무 — 모든 상업 장르 공통]

  □ 1씬에 대사가 1줄 이상 있는가?
  □ 그 대사가 첫 3분 안에 등장하는가? (1씬 끝에 한 줄 끼우는 식 금지)
  □ 그 대사가 작품의 톤·관계·정보·미스터리 중 하나를 선언하는가?
  □ 대사 없이 환경 묘사 + 인물 행동만으로 1씬이 구성되지 않았는가?
  → 1씬 대사 부재는 자동 위반. 무조건 재집필.

[자가 검토 — 무성영화 테스트]

  Q. 이 1씬을 무성영화로 만들면 같은 효과가 나는가?
     → YES면 아트하우스 문법. 상업 장르 영화는 NO여야 정답.
  Q. 첫 1분 동안 관객이 인물의 목소리를 한 번이라도 들었는가?
     → NO면 동일시 실패. 무조건 재집필.

[장르별 1씬 대사 형식 권장]

호러: Cold Open Dialogue / Phone Call / Witness Dialogue / Mystery Hook
   (HORROR DNA #6 1씬 대사 4유형 참조)

액션: Banter / Threat Declaration / Briefing
   - 동료 간 농담 또는 위협 선언 또는 작전 브리핑
   - 액션 중에 던지는 한 줄도 충분 ("Get down!" "엎드려!")
   - 레퍼런스: 〈베테랑〉 도철 농담 / 〈범죄도시〉 마석도 위협 / 〈매드맥스〉 맥스 내레이션

스릴러 / 범죄: Information Asymmetry Dialogue / Clock Dialogue
   - 정보 비대칭을 선언하는 대사 또는 시한을 명시하는 대사
   - 레퍼런스: 〈테이큰〉 첫 전화 / 〈올드보이〉 "누구냐 넌" / 〈양들의 침묵〉 첫 인터뷰

코미디: Hook Joke / Self-Reveal Dialogue
   - 첫 대사가 첫 웃음 또는 캐릭터 코믹 결함 자기 폭로
   - 레퍼런스: 〈극한직업〉 형사 대화 / 〈써니〉 임나미 자기소개

로맨스 / 멜로: Tone Dialogue / Wound Dialogue
   - 사랑에 대한 태도를 한 줄로 선언 또는 상처의 흔적이 드러나는 대사
   - 레퍼런스: 〈비포 선라이즈〉 기차 대화 / 〈건축학개론〉 회상 대사

SF: World Rule Dialogue / Technology Reaction
   - 세계 규칙을 일상 대화로 흘리거나 기술 반응 대사
   - 레퍼런스: 〈매트릭스〉 "Wake up, Neo" / 〈블레이드 러너〉 인터뷰

판타지: Mythic Hook / Threshold Dialogue
   - 신화적 톤 선언 또는 경계 넘기 대사
   - 레퍼런스: 〈반지의 제왕〉 갈라드리엘 내레이션 + 첫 대사 / 〈해리포터〉 더즐리 일상 대사

[금지 패턴 — 모든 상업 장르 공통]
❌ 1씬 전체 대사 0줄
❌ 1씬 후반에 한 줄 끼워넣기 ("막판 대사 알리바이")
❌ 환경 묘사 + 인물 단독 행동만으로 1씬 구성
❌ 내레이션(보이스오버)만 있고 캐릭터 간 대사 부재
❌ 의미 없는 일상 단어("어?" "음...") 단발성으로 룰 회피

[예외 — 정통 드라마만 침묵 도입 허용]

정통 드라마(액션·스릴러·코미디·로맨스 요소가 메인이 아닌 작품)는
침묵 도입이 작법적으로 허용된다. 다만 다음 조건을 모두 충족해야 한다:
  1. 작가가 명시적으로 침묵 도입을 의도함
  2. 침묵이 작품 톤과 일치함 (느린 호흡의 드라마)
  3. 침묵 후 3분 안에 결정적 대사 1줄 등장
조건 미충족 시 드라마도 대사 의무 적용.
...
▸ 액션 → 권장 기법: Action Drop / Character Reveal Action
   ...
   - ★ v3.7.2: 1씬 대사 1줄 이상 필수 (절대 룰 블록 참조)
...
▸ 드라마 → 권장 기법: Character Reveal Action / Hook Dialogue
   ...
   - ★ v3.7.2: 침묵 도입 허용 (정통 드라마 한정). 단 침묵 후 3분 안에 결정적
     대사 1줄 등장 필수. 다른 장르 요소가 메인이면 1씬 대사 의무 적용.
...
▸ 코미디 → 권장 기법: Character Reveal Action / Hook Dialogue
   ...
   - ★ v3.7.2: 1씬 대사 1줄 이상 필수. 코미디는 특히 첫 대사가 첫 웃음.
...
▸ SF → 권장 기법: Cold Open / Tease & Reveal
   ...
   - ★ v3.7.2: 1씬 대사 1줄 이상 필수 (설명 대사 금지지만 인물 대사는 필수)
...
▸ 판타지 → 권장 기법: Cold Open / Tease & Reveal
   ...
   - ★ v3.7.2: 1씬 대사 1줄 이상 필수 (긴 세계관 내레이션과는 다름)
...
❌ ★ v3.7.2: 상업 장르 영화 1씬에 대사 0줄 (아트하우스 영화 문법)

이 습관은 소설·교과서·아트하우스 영화의 문법이지 상업 장르 영화의 문법이
아니다. 첫 3줄에 반드시 6기법 중 하나가 작동해야 하고, 첫 3분 안에 반드시
대사 1줄이 등장해야 한다 (정통 드라마 제외).
...
18. AI ESCAPE — 집필 후 A1~A29 패턴 자가 점검:
   ...
   POV 회전 강제(A27) /
   ★ v3.7 추가: 씬 번호 연속성(A28) / 지문 단락 과잉 분단 방지(A29)
   — 1개라도 해당되면 다시 써라.
