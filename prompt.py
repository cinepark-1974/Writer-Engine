...
9. 감정 연쇄:
   - 이 씬의 감정 톤이 다음 씬의 전제가 된다. 3%라도 빗나가면 엔딩이 무너진다.
   - 헤어지는 장면의 감정, 선택의 무게, 대가의 크기 — 전체 설계와 정확히 일치시켜라.
   - 캐릭터의 구체적 디테일(습관, 공간, 물건)이 장면을 살린다. 추상적 설정 금지.
10. Intention & Obstacle:
   - 매 씬에서 "누가 뭘 원하고, 뭐가 막는가"를 명확히. 이것 없으면 씬을 시작하지 마라.
   - 장애물을 넘는 전술(위협/논리/유혹/회피)이 캐릭터를 정의한다.
11. "Too Wet" 금지:
   - 관객이 느낄 감정을 캐릭터가 직접 수행하지 마라.
   - 감정을 참는 캐릭터가 터뜨리는 캐릭터보다 강하다.
