# ─────────────────────────────────────────────────────────────
# BLUE JEANS SCREENPLAY WRITER ENGINE v3.15.0
...
# ─────────────────────────────────────────────────────────────
#
# v3.14.0 주요 변경사항 (2026-08-22):
# - A35 신설 (대사의 핑퐁 리듬 — Ping-Pong, No Monologue) — SYSTEM_PROMPT 전역.
#   * Mr. MOON 진단: "범인에게 형사가 일방 연설은 안 된다. 핑퐁 대화가
#     로코에 반드시 필요하고, 스릴러도 마찬가지다."
#   * 핵심: 일방 연설 금지(장르 불문). 상대는 듣는 벽이 아니라 되받아치는 상대.
#   * 대사 겹침(오버랩)은 명시적으로 배제 — 한국 관객은 대사 전달에 예민
#     (검색 확인: 2010년대 후반부터 "대사 안 들린다" 불만 도배, 한글자막 상영관
#     수요 증가). 겹침 대신 "겹치지 않되 딜레이 없는 즉각 받아치기".
#   * 긴 대사 예외: "그 말하기 자체가 판을 뒤집는 행동"일 때만(A32/A33 기준).
#
# - ROMCOM 부스터에 SCREWBALL TIKITAKA 블록 신설 (전 비트 적용).
#   * Mr. MOON 진단: "연애.결혼이 그게 부족하다. 로코 시나리오가 밋밋하면 안 된다.
#     스크루볼 코미디가 유독 한국에서 강하다."
#   * 근본 원인: 기존 티키타카 규칙(scene_sequence.py)이 1막 셋업(ACT1_ROMCOM)에만
#     갇혀 있었다. 2·3막에서 티키타카 강제가 사라져 로코 대사가 다시 밋밋해짐.
#   * 해결: 로코 자체를 스크루볼 강도로 상향(별도 _is_screwball 판별 없이 로코 전체).
#     한국 로코는 스크루볼 핏이 기본값이라는 Mr. MOON 판단 반영.
#   * 티키타카 3조건(말버릇 대비/받아치기/판돈) 전 비트 확장 + 스크루볼 강도
#     (짧은 연쇄·과장 되받기·연쇄 Topper·자존심 싸움) + 전 비트 자가검증.
#
# - 대사 길이 기준을 레이아웃에서 산출(추론 아님).
#   * Mr. MOON 지적: "세 줄"은 시나리오 대사 정렬 폭 기준 글자 수로 확정해야 함.
#   * main.py 대사 스타일 실측: A4 210mm, 좌우 20mm, 대사 들여쓰기 1.25cm,
#     함초롬바탕 10pt → 대사 영역 157.5mm → 한 줄 약 45자.
#   * 사의재 원고 실제 대사 280개 분포로 교차검증(평균 16.6자, 40~49자가 2줄 경계).
#   * 확정: 로코 티키타카 기본 한 줄(45자)/최대 두 줄(90자)/세 줄(135자)은
#     "말하기가 사건"일 때만.
#
# - 검증 방식은 A34와 동일(해석1, 1콜 자가 검문). API 콜 추가 없음.
# - main.py 수정 불필요 — 전부 prompt.py 내부(SYSTEM_PROMPT + ROMCOM 부스터 상수).
#   함수 시그니처 불변(백워드 호환 유지). 기존 scene_sequence.py 티키타카 미변경.
#
# - 참고 자료(엔진 미내장): WGA 101 시나리오 리스트 대조로 "우리가 놓친 것" 도출.
#   * 대조 결과: 되돌릴 수 없는 변화·서브텍스트·캐릭터 결함·소품·반전 등 98편의
#     기법은 이미 엔진에 존재. 빈 것은 아이러니/정보통제/대사리듬 3개.
#   * 정보 통제(죠스식 늦은 공개)는 의도적 제외 — Mr. MOON 판단: 2010년대 이후
#     한국 상업영화는 정보를 아끼는 서스펜스보다 부딪히고 밀어붙이는 "사이다"가
#     흥행 공식. 정보 통제는 호러 부스터 국소 적용 여지만 메모(이번 미적용).
#   * 대사 리듬 → A35 + 스크루볼 블록으로 이번에 반영. 아이러니는 향후 과제.
#
# ─────────────────────────────────────────────────────────────
#
# v3.13.0 주요 변경사항 (2026-08-22):
# - A34 신설 (대사의 AI 과잉 말투 제거 — Subtext & Spoken Voice Mandate).
#   * Mr. MOON 진단: "결과물 대사가 AI 과잉 말투다. 뜻을 다 말하고, 전원이
#     매끈한 표준 문어체를 쓴다. 사투리가 아예 없다."
#   * 실측 근거: 사의재(정약용) 원고 — 강진 주모가 표준 서울말/문어체로 말함.
#     작가가 일부 수동 교정("얼굴이 가칠하니 뭔 일 있데?", "쓸라면 쓰소잉")했으나
#     엔진이 처음부터 구어로 뽑았다면 불필요했을 작업.
#   * 근본 원인: 기존 자가 점검 ①~⑥은 지문(사건·인과·샷·현재형)만 검증.
#     대사가 "설명"인지 "말"인지 보는 층위가 없었다. 잘 쓰인 문어체 대사가
#     검문을 통과했다.
#
# - A34 네 검문:
#   * 검문1 역전 — 겉말=속뜻이면 서브텍스트 없음(설명). 형식과 내용을 어긋내라.
#   * 검문2 감정 우회 — 감정 직접 설명 금지. 딴소리·행동·침묵으로.
#   * 검문3 말맛 — 같은 뜻의 더 짧고 입에 붙는 구어. (드라마/아트하우스 완화 가능)
#   * 검문4 완결 — 병렬 대칭 완결 금지. 흘림·끊김·되묻기 허용.
#   * 대전제: 대사는 배우가 소리 내는 말. 표준 문어체 완성문 수렴 금지.
#   * 균형 안전장치: 단서 없는 서브텍스트=아트하우스 대사도 실패(상업 시나리오).
#
# - 말투 변형(사투리·고어·외국어)은 "강제 조항"일 때만 — 인물 설정에 명시적
#   근거가 있을 때만 반영. 근거 없으면 표준 구어체 유지, 억지 변형 금지.
#   * 사투리는 명사 치환이 아니라 어미·종결·리듬으로. 가짜 사투리 금지.
#   * 이유: "전라도 출신이어도 서울말 쓰는 영화가 있다"(Mr. MOON). 출신지가
#     곧 사투리 강제가 아니다. 최종 방언 고증은 작가 몫, 엔진은 구어 리듬까지.
#   * 캐릭터 말투 필드는 향후 Creator Engine 연동 예정 — 이번엔 Writer가
#     기본 구어체 토대를 만드는 데 집중(좋은 구어가 있어야 사투리 수정이 가능).
#
# - 자가 점검 체크리스트에 ⑦ 추가 — "대사가 서브텍스트를 가졌는가/구어인가"
#   (겉말!=속뜻, 배우가 소리 내는 말 / A34). ①~⑥ 통과해도 ⑦ 없으면 재집필.
#   * 검증 방식: 해석1(1콜 자가 이중 검문). A32/A33과 동일 구조, API 콜 추가 없음.
#
# - 참고 자료(엔진 미내장, 작가 검수용): 신스텔러 대사론(텍스트/서브텍스트),
#   AFI 100 Movie Quotes, 한국 명대사 10선, 국립국어원 지역어·WORDROW 사투리 사전.
#   * 원리만 추출하고 걸작 대사는 직접 인용하지 않음(Mr. MOON 지침).
#   * 방대한 방언 사전은 명사 위주라 미내장 — 대사는 어미가 핵심, 데이터 주입 시
#     가짜 사투리·크레딧 폭증 위험.
#
# - main.py 수정 불필요 — A34는 SYSTEM_PROMPT 인라인, 체크리스트 확장뿐.
...
#   * Mr. MOON 진단: "법무팀장(50대, 넥타이 단정) 식으로 표기되는데, 김지훈
#     (30대, 남)처럼만 묘사할 수 없나? 외양·복식은 캐스팅·미술·의상팀·감독이
#     정할 영역이다."
#   * 작법 원칙 — 시나리오는 설계도이지 완성 그림이 아니다. "누가"(나이대·성별)
#     까지가 작가의 일이고, "어떻게 보이는가"(넥타이·정장·인상·헤어·체격)는
#     캐스팅·미술·의상·분장·연출의 재량. 괄호에 외양을 박으면 월권.
#   * 첫 등장 괄호를 (나이대, 성별) 2개 항목으로 제한. 외양·복식·표정·소품 금지.
#   * 예외 — 외양 디테일이 서사에 직접 기능(플롯 단서·관계/계급 장치)할 때만
#     괄호가 아니라 지문 본문 문장으로 푼다.
#   * 18번 AI ESCAPE에 ⑭ 체크 + 리스트 헤더 A1~A30 → A1~A31 + 끝항목 추가.
#   * SYSTEM_PROMPT 인라인 룰이므로 전 작품 항상 적용. 함수 시그니처·main.py
#     호출부 무변경, 구버전 100% 호환.
#
# v3.8.1 주요 변경사항 (2026-06-20):
# - A14 강화 — 풀네임 → 이름 전환 강제 (작법 표준 명문화)
#   * Mr. MOON 진단: "주인공 이름이 성까지 다 나오는 게 맞나? 김지훈(30대,남)이
#     처음이고, 그 다음부터는 지훈은… 이렇게 하는 게 낫다."
#   * '상속(가제)' v3.8.0 결과물 진단 — 강도현 풀네임 812회 / 이름 단독 0회,
#     한석훈 346회 / 0회. 이름만 쓴 경우가 전무. A14가 (나이/직업) 괄호 반복만
#     막고 풀네임 반복은 방치하던 허점을 보강.
#   * A14를 가(괄호 반복)·나(풀네임 전환) 2부 구조로 재편. 첫 등장 1회만
#     성+이름, 이후 전부 성을 뗀 이름. 지문·대사 헤더 양쪽 적용.
#   * 예외 3종 명시(동명이인 / 직책 호칭 중요 / 격식·거리감 의도).
#   * 18번 AI ESCAPE A14 라벨 갱신("캐릭터 재소개·풀네임 반복").
#   * 룰 본문만 교체 — 함수 시그니처·main.py 호출부 무변경, 구버전 100% 호환.
#
# v3.8.0 주요 변경사항 (2026-06-20):
# - A30 미시 연출 반복 차단 + 작가주의/상업 자동 분기 신설
#   * Mr. MOON 진단: "Creator가 작가주의를 분리했다. 상업영화 시나리오인데
#     볼펜 뚜껑·주머니에 손 넣기·5초간 멈춤 같은 미시 연출이 너무 자주 나온다.
#     두 엔진이 유기적으로 돌아가야 이걸 막을 수 있다."
#   * Creator v2.6.1(사전 방지: 톤 문서에 미시 연출 미생성)과 짝을 이루는
#     Writer 측 사후 차단 룰. 비트 집필·재집필 시 자가 차단.
#
# - _detect_arthouse_writer() 신규 — 작가주의/예술영화 자동 감지
#   * 키워드 셋은 Creator v2.6.1 _detect_arthouse()와 동일(두 엔진 동기화)
#   * 입력 필드(genre+logline+world+treatment+story_elements) 합쳐 스캔
#   * 상업영화(기본) → A30_DIRECTION_REPETITION_BLOCK 주입
#   * 작가주의       → A30_ARTHOUSE_LICENSE 주입(미시 연출 허용)
#
# - A30 룰 3유형 차단 (상업영화 한정 / 작가주의 면제):
#   * 유형1: 서사와 무관한 오브제의 비트 간 반복 (Setup-Payoff 등재 항목만 허용)
#   * 유형2: 정형 습관 동작 반복 (주머니에 손·펜 뚜껑·한숨·머리 쓸어넘김 등)
#   * 유형3: 초 단위 멈춤 연출 ("5초간 멈춘다"·"0.5초 정지" 등 연출 영역 침범)
#
# - 18번 AI ESCAPE: A30 항목 추가 (총 A1~A30 점검), 8점 진입 체크 ⑪⑫⑬ 추가
# - build_write_beat_prompt / build_targeted_rewrite_prompt 양쪽에 A30 적용
#   * 시그니처 무수정 — 기존 입력 필드만 스캔하므로 main.py 호출부 변경 없음
#   * 구버전 호출 100% 호환
#
# ─────────────────────────────────────────────────────────────
#
...
#   공통으로 발견된 2종 결함을 사전 차단
#   * Mr. MOON 진단: "7.7점에서 8점을 못 넘는 이유는 비트 하나가 아니라
#     비트 사이의 연결 패턴 때문이다. A25~A27이 비트 내부의 룰이라면,
#     A28~A29는 비트 사이의 룰이다."
#
# - A28: 행동 사이클 반복 차단 (Action Cycle Loop Block)
#   * A26과의 차이: A26은 공간 카운트, A28은 행동 시퀀스 패턴
#   * 직전 2비트의 [공간 순서 + 사건 기능] 비교 강제
#   * 사이클 변주 4기법: 순서 역전 / 압축 / 차단 / 침범
#   * 조력자 만남의 일회성 원칙 (같은 공간 3회째부터 변주 강제)
#   * "또 X냐" 자가 점검
#
# - A29: 물리적 대가 에스컬레이션 (Physical Cost Escalation)
#   * 대가는 타인(조력자)이 아닌 주인공 자신의 몸에
#   * 4단계 누적 에스컬레이션 (일시→가시→영구→기능 손상)
#   * 비트 구간별 단계 매핑 (1~5/6~8/9~11/12~)
#   * 장르별 대가의 결 (호러/스릴러/액션/범죄/드라마/로맨스)
#   * 누적성 원칙 — 이전 대가는 사라지지 않아야 한다
#
# - 14번 지문 룰: 8점 진입 체크 ⑨, ⑩ 추가
#   * ⑨ 행동 사이클 반복 차단 / ⑩ 물리적 대가 에스컬레이션
#
# - 18번 AI ESCAPE: A28, A29 항목 추가 (총 A1~A29 점검)
#
# - 적용 범위: build_write_beat_prompt + build_targeted_rewrite_prompt
#   양쪽 모두 자동 적용 (두 함수 모두 14번/18번 룰을 공통 사용)
#
# - Creator Engine v2.5.5와의 업무 분장 (예정):
#   * Creator: 기획 단계 사전 방지 (루프 없는 시퀀스 설계, 능동 적대자,
#     의도적 Plant/Payoff, 대가 에스컬레이션 계획)
#   * Writer: 집필 단계 사후 감지·교정 (A28, A29 자가 점검)
#
...
#   * 문제: AI가 A22를 과해석해 같은 인물 같은 방향 동선까지 끊어버림
#     "수현이 서점 문을 닫고 나온다. 골목 계단 중간에 앉는다." (3개 문장 끊김)
#   * 해결: "수현이 서점 문을 닫고 나와 골목 계단 중간에 앉는다." (1개 문장)
#   * BAD/GOOD 예시 4쌍 추가 (서점→계단, 일어서기→이동→커피머신,
#     가방→문→나가기 등)
#   * 판별 기준 4가지 명시:
#     1) 같은 인물 같은 방향 이동 2) 한 호흡 연속 동작
#     3) 카메라 follow 한 컷 4) 의미상 한 사건의 부분들
#   * 적용 결정 트리(Q1→Q2→Q3) 추가
#   * 14번 지문 룰에도 같은 동선 묶기 BAD/GOOD 예시 직접 추가
#
# v3.5.1 주요 변경사항 (2026-05-04):
# - 한국 시나리오 표준 포맷 — 지문↔대사 빈 줄 강제 (가독성 개선)
#   * prompt.py: OUTPUT FORMAT에 빈 줄 강제 명령 추가 (1차 안전망)
#   * main.py: DOCX 렌더 시 prev_block_type 추적해 자동 빈 줄 삽입 (2차 안전망)
#   * main.py: TXT 출력 전 _normalize_screenplay_blank_lines() 후처리 (3차 안전망)
#   * 같은 대사 블록(같은/다른 화자) 안에서는 빈 줄 없이 연속 유지
#
# v3.5.0 주요 변경사항 (2026-05-04):
# - "오랜만에(가제)" 작품의 Rewrite 7.1점 진단 + Revise 6.8점 검증보고서 분석
#   결과를 반영. 7점 안정 진입을 위한 룰 3종 신설.
#
# - A19. 시간 정밀 표기 금지 (대폭 강화)
#   * "0.3초", "0.5초", "1초", "3초" 등 숫자 시간 표기 절대 금지
#   * 시간 정밀 표기는 작가가 아닌 연출자의 영역
#   * 본문 GOOD 예시 7개에서 시간 표기 모두 제거 (L193, L981, L1052,
#     L1152, L4238, L5283, 침묵 표기)
#   * 모호한 시간어("잠깐", "한참", "[긴 정적]")로 대체
#
# - A25. CHARACTER VOICE DIFFERENTIATION (신규)
#   * 한 인물의 회피·동의·놀람·침묵 어휘 3회 이상 동일 사용 금지
#   * 인물별 회피 어휘 풀 4가지 이상 강제
#   * 인물별 시그니처 주저 패턴 차별화
#   * 인물별 평균 문장 길이 차별화
#
# - A26. MIDPOINT EROSION 차단 (신규)
#   * Beat 6~10 정체 차단 — AI 시나리오 최대 결함
#   * 메인 공간 4회 이상 연속 등장 금지
#   * 거부/장애물의 4단계 질적 변주 (무시→반격→전복→폭로)
#   * B스토리 능동 진입 강제
#   * 씬 길이 변주 강제 (단편/중편/장편 혼합)
#   * 중간점(Beat 8) 결정타 강제
#
# - A27. POV ROTATION ENFORCER (신규)
#   * A18(단일 시점 고정 금지) 약함 → 작품 전체 POV 균형 강제
#   * 메인 인물 2명 작품: 인물 A ≤70%, 인물 B ≥25%
#   * 양쪽 단독 씬 최소 2씬씩
#   * 시선의 호혜성 강제 (관찰 횟수 균형 ±30% 이내)
#
# - GENRE_BOOSTER_ROMCOM 보강 — OBSTACLE INTENSITY 모듈 추가
#   * 만남 빈도 50% 상한
#   * 만나지 못함 씬 20~30% 강제
#   * 정보 비대칭 최소 Beat 3개 유지
#   * 장벽의 에스컬레이션 (Beat 6, 10, 13)
#
# - 14번 지문 룰: 시간 표기 금지 + A25~A27 체크리스트 통합
# - 18번 자가 점검 룰: A1~A24 → A1~A27로 확장
#
...
#   * A21. 인과의 사슬 (Causality, Not List) — McKee Archplot
#   * A22. 한 문장 = 한 샷 (One Sentence, One Shot) — Tony Tost
#   * A23. 현재형 능동 (Present Tense, Active Voice) — MasterClass 표준
#   * A24. 동사 없는 단편 (Verbless Fragments for Close-ups) — Alien (1979)
# - 18번 자가 점검 룰 확장: A1~A20 → A1~A24
# - 14번 지문 룰에 헐리우드 작법 5체크 통합 (visualizable / 인과 / 1문장1샷 /
#   현재형 능동 / 비트=문단)
# - 19번 룰(카메라 매뉴얼체 금지)과 A22(1문장1샷) 역할 분리 명시:
#   * 19번 = 출력 형식(줄바꿈) 문제
#   * A22 = 의미 단위(샷) 문제
# - Revise Engine v2.6과 동일 작법 룰셋 사용 → 두 엔진 출력 톤 통일
#
# v3.2.1 주요 변경사항 (2026-04-26):
# - GENRE BOOSTER 7종 추가 (총 5종 → 12종)
#   COMEDY (단독) / MYSTERY (추리) / FANTASY / SF / PERIOD (사극) /
#   COMING_OF_AGE (청춘·성장) / DISASTER (재난)
# - 신규 판별 함수 7종 (_is_comedy_only / _is_mystery / _is_period /
#   _is_coming_of_age / _is_disaster) — _is_sf, _is_fantasy는 기존 활용
# - 부스터 자동 선택 우선순위 재구성 (단독 코미디 추가, 결합 케이스 대응)
# - 자가 검증 게이트 7종 신규 룰 추가
# - Period Pack과 PERIOD 부스터 역할 분리:
#   * Period Pack = 시대 디테일 (복식·관직·통화·언어결)
#   * PERIOD 부스터 = 사극 비트 코어 DNA (권력 위계·의례·시대 사건)
#
# v3.2.0 주요 변경사항 (2026-04-26):
# - GENRE BOOSTER MODULE 5종 신설 (ROMCOM/THRILLER/HORROR/DRAMA/ACTION)
# - HELPER CHARACTER RULE 신설 — 조력자 캐릭터 자립성 강제
# - 비트 종료 시 GENRE_BOOSTER_CHECK 자가 검증 게이트
# - _is_romcom() 판별 함수 신규 (코미디+로맨스 결합)
# - 매 비트 자가 점검 5개 룰 + 미달 시 재집필 강제
#
...
✅ 신체 반응 설계: "재중의 손가락이 USB에 닿는다. 멈춘다. 호흡이
...
지문에 명시적 지시를 써라: "[긴 정적]", "[잠깐의 침묵]", "[침묵]", "답하지 않는다."
※ "N초" 같은 정밀 시간 표기는 연출자의 영역이지 작가의 영역이 아니다 — 사용 금지.
...
# ═══════════════════════════════════════════════════════════
# ★ v3.8.0 NEW — A30 작가주의/상업영화 자동 분기 + 미시 연출 반복 차단
# ═══════════════════════════════════════════════════════════
#
# 배경:
#   Creator Engine v2.6.1이 작가주의/상업영화를 판별해, 상업영화면
#   톤 문서에 미시 연출(볼펜 뚜껑 여닫기·주머니에 손 넣기 등)을 아예
#   생성하지 않도록 사전 방지하기 시작했다(DIRECTION_AUTHORITY_BOUNDARY).
#   그러나 Creator가 안 넣어도, Writer가 비트 집필 중 스스로 그런 미시
#   연출을 생성하면 같은 동작·오브제가 작품 전체에 반복된다.
#   따라서 Writer 측에도 같은 기준의 사후 차단 룰이 필요하다(Mr. MOON 운영 중 발견).
#
# 업무 분장:
#   - Creator (사전 방지): 톤 문서에 미시 연출을 생성하지 않음
#   - Writer  (사후 차단): 비트 집필 시 미시 연출 반복을 자가 차단 (A30)
#
# 분기:
#   - 상업영화 감지 → A30_DIRECTION_REPETITION_BLOCK 주입 (미시 연출 반복 차단)
#   - 작가주의 감지 → A30_ARTHOUSE_LICENSE 주입 (시그니처 오브제·미시 연출 허용)
#
# 감지 키워드 셋은 Creator v2.6.1 _detect_arthouse()와 동일하게 유지(엔진 동기화).
# 함수 시그니처 무수정 — build_write_beat_prompt가 이미 받는 입력 필드를 스캔.
# ═══════════════════════════════════════════════════════════

# Creator v2.6.1 _detect_arthouse()와 동일 셋 — 두 엔진 동기화 (수정 시 양쪽 함께)
_ARTHOUSE_KEYWORDS = [
    # 작가주의·예술영화 명시
...
def _detect_arthouse_writer(scan_text: str) -> bool:
    """작가주의/예술영화 작품 자동 감지 (Writer 측).

    Creator v2.6.1 _detect_arthouse()와 동일 키워드 셋을 사용해
    두 엔진이 같은 기준으로 판별하도록 한다.
    scan_text는 build_write_beat_prompt가 이미 받는 입력 필드
    (genre + logline + world + treatment + story_elements)를 합친 것.

    True  → 작가주의 → A30_ARTHOUSE_LICENSE 주입 (미시 연출 허용)
    False → 상업영화(기본) → A30_DIRECTION_REPETITION_BLOCK 주입 (반복 차단)

    v3.8.0: 신규. '볼펜 뚜껑 여닫기'·'주머니에 손 넣기'·'5초간 멈춤' 같은
            독립영화풍 미시 연출이 상업 시나리오 전반에 반복 주입되던 결함
            (Mr. MOON 운영 중 발견)을 작가주의 작품에만 한정하기 위한 트리거.
    """
    if not scan_text:
        return False
    for kw in _ARTHOUSE_KEYWORDS:
        if kw in scan_text:
            return True
    return False


# ── 상업영화 기본 — 미시 연출 반복 차단 룰 (A30)
A30_DIRECTION_REPETITION_BLOCK = """
[A30. 미시 연출 반복 차단 — Direction Repetition Block (상업영화 / 자동 주입)]

이 작품은 상업영화로 판단되었다. 작가주의 신호가 감지되지 않았으므로,
아래 세 유형의 '연출 영역 침범 + 반복'을 차단하라. 이것들은 작가가 아니라
배우·감독이 현장에서 만들 영역이며, AI가 내면 표현 대용으로 자동 반복하면
작품 전체에 같은 동작·오브제가 깔려 톤이 작가주의로 흘러간다.

A20(같은 씬 안 같은 명사 3회 차단)이 '씬 내부' 룰이라면,
A30은 '작품 전체(비트 간)' 룰이다 — 역할이 다르다.

─────────────────────────────────────
유형 1 — 서사와 무관한 오브제의 반복 등장
─────────────────────────────────────
맥거핀·복선(Setup-Payoff 테이블 등재 항목)이 아닌데도 같은 소품이
여러 비트에 계속 등장하는 것. AI가 '분위기'를 위해 자동으로 꺼내 쓰는 소품.

❌ BAD — 서사 기능 없는 오브제 반복:
  Beat 3:  민수가 볼펜 뚜껑을 연다.
  Beat 5:  민수가 볼펜 뚜껑을 딸깍 닫는다.
  Beat 7:  볼펜 뚜껑을 만지작거린다.
  → 볼펜은 복선도 맥거핀도 아니다. 서사와 무관한 장식적 반복. 차단.

✅ GOOD — 등재된 복선·맥거핀만 반복:
  Setup-Payoff 테이블에 '아버지의 라이터'가 등재되어 있다면,
  그 라이터는 의도된 회수를 위해 반복 등장해도 된다. (그게 복선의 기능)

원칙:
  - Setup-Payoff 테이블에 없는 미시 소품(볼펜·라이터·컵·열쇠고리 등)은
    작품 전체에서 서사 기능 없이 반복 등장시키지 마라.
  - 소품을 등장시키려면 "이게 복선인가, 회수될 것인가"를 먼저 물어라.
    아니라면 그 장면 한 번으로 끝내라.

─────────────────────────────────────
유형 2 — 반복되는 습관 미시 동작
─────────────────────────────────────
AI가 인물의 긴장·생각·주저를 표현할 때 자동으로 꺼내 쓰는 정형 동작.
같은 동작을 비트마다, 게다가 인물 가리지 않고 돌려쓰면 클리셰가 된다.

❌ BAD — 정형 습관 동작의 반복 (AI가 남발하는 대표 목록):
  · 주머니에 손을 넣는다
  · 볼펜(펜) 뚜껑을 여닫는다 / 펜을 돌린다
  · 머리(머리카락)를 쓸어넘긴다
  · 한숨을 (길게) 쉰다
  · 입술을 깨문다
  · 안경을 고쳐 쓴다
  · 손가락으로 책상을 두드린다
  → 위 동작들을 '긴장 = 이 동작' 식으로 자동 대입해 반복하지 마라.

✅ GOOD — 인물 고유 행동으로 차별화:
  긴장·주저는 그 인물만의 구체적 행동으로 표현한다.
  요리사라면 칼을 도마에 내려놓는 방식으로, 형사라면 수첩을 덮는 방식으로.
  같은 감정도 인물마다 다른 행동으로 나타나야 한다.

원칙:
  - 위 정형 동작을 작품 전체에서 한 인물당 반복 사용하지 마라.
  - 서로 다른 인물에게 같은 습관 동작을 돌려쓰지 마라.
  - 내면(긴장·불안·고민)을 동작으로 옮길 때는 그 인물·그 장면 고유의
    행동을 새로 설계하라. 기성 동작 목록에서 꺼내지 마라.

─────────────────────────────────────
유형 3 — 시간 정밀 표기를 동반한 멈춤 연출
─────────────────────────────────────
"반응을 5초간 쉬었다 한다", "0.5초 정지", "3초간 침묵" 같은 초 단위 멈춤.
멈춤의 '길이'를 숫자로 처방하는 것은 작가가 아니라 연출자(감독·편집)의 영역이다.

❌ BAD — 초 단위 멈춤 연출:
  민수가 그녀를 본다. 5초간 멈춘다. 그리고 답한다.
  (0.5초 정지) / (3초 무음, 환경음만 남는다)
  → 몇 초 멈출지는 배우·감독·편집이 현장에서 만든다. 작가가 못 박지 마라.
...
원칙:
  ...
  - 멈춤은 행동의 부재(답하지 않는다·움직이지 않는다)나
    다른 행동(시선 이동·자리 이동)으로 표현한다.
  - 객관적 세계 시간(마감 D-3, 9시 정각, 타이머 화면)은 예외 — 허용.

[핵심]
상업영화의 지문은 "인물이 무엇을 하는가, 무엇이 보이는가"를 쓰고,
"그 동작을 몇 초 동안 어떻게 반복할 것인가"는 쓰지 않는다.
후자는 배우·감독·편집의 영역이다.
"""


# ── 작가주의 — 미시 연출·시그니처 오브제 허용 라이선스 (A30)
A30_ARTHOUSE_LICENSE = """
[A30. 작가주의 연출 라이선스 — Arthouse Motif License (작가주의 작품에만 자동 주입)]

이 작품은 작가주의/예술영화로 자동 감지되었다.
이 작품에 한해, 시그니처 오브제·반복 모티프·절제 미학 연출을 허용한다 —
작가주의 영화는 연출 디테일 자체가 작품의 본질이기 때문이다.

[허용]
  - 시그니처 오브제(반복 소품)와 그 변주·최종 회수 설계.
  - 반복되는 미시 동작을 인물의 정체성·내면의 시그니처로 활용.
  - 절제 미학·침묵·미시 신체 반응의 정밀한 연출.

[주의]
  - 연출 디테일이 '작가의 시그니처'로 기능하도록 설계하라.
    주제·인물 내면과 직결된 오브제·동작만 반복하라. 장식적 반복은 여전히 금지.
  - 초 단위 시간 표기(N초 멈춤)는 작가주의 작품이라도 권장하지 않는다.
    멈춤의 길이는 연출·편집의 영역이며, 작가는 '무엇을 하지 않는가'로 표현한다.
"""
...
# ═══════════════════════════════════════════════════════════
# ★ v3.2.0 NEW MODULE — GENRE BOOSTER (5종)
# 장르별 비트 강제 룰 + 자가 검증 게이트.
# 사용자 v3.2 업그레이드 요청서 사양 반영.
# 매 비트 집필 시 장르 코어 DNA 룰 5개 중 N개 충족 강제.
# ═══════════════════════════════════════════════════════════

GENRE_BOOSTER_ROMCOM = """
━━━━━━━━━━━━━━━━━━━━━━━━━━━
GENRE BOOSTER — ROMCOM (로맨틱 코미디 비트 강제 규칙 v3.2)
━━━━━━━━━━━━━━━━━━━━━━━━━━━

★ 매 비트 집필 시 다음 5가지 중 최소 2개를 비트 안에 강제 배치하라.
★ 코믹 폭발 포인트는 한 비트에 최소 1회, 최대 3회.

[로코 코어 DNA 5종]

1. 코믹 곤혹 상황 (Embarrassment Sequence)
   - 주인공이 들킬까 봐 황급히 숨기는 무언가
   - 우연한 신체 접촉 직후의 어색함
   - 잘못된 타이밍의 등장/말실수
   - 예: 유진이 인스타로 수강생 평가하다 통화 중인 것을 들킴

2. Misdirection (오해 폭소)
   - 한 인물이 A를 말하는데 다른 인물은 B로 듣는 구조
   - 서로 다른 맥락으로 진행되다 만나는 순간 폭소
   - 예: '재료가 좀 어려워요' → 수강생은 본인 얘긴 줄 모름

3. Comic Specificity (구체적 디테일이 만드는 웃음)
   - 막연한 '긴 시간'이 아닌 '9분'
   - 막연한 '음식'이 아닌 '면이 불은 파스타'
   - 구체적 숫자/사물명이 코미디 핵심
   - 예: '한도 80% 도달' 같은 정확한 수치

4. Status Flip (지위 역전)
   - 권위자가 우스워지는 순간
   - 무능해 보이던 인물이 정확한 통찰을 던지는 순간
   - 어색하던 인물이 주도권 잡는 순간
   - 예: 7세 어린이의 '그냥 먹으면 되잖아요'

5. Topper (한 번 더 웃기기)
   - 첫 웃음 직후 곧바로 두 번째 펀치 (한 박자 안에)
   - 어린이/조연의 무자비한 펀치
   - 예측 가능한 답변에 한 번 더 비틂

[★ 자가 검증 — 비트 종료 시 점검 (5개 중 3개 이상 충족)]

  □ 이 비트에 코믹 폭발 포인트가 최소 1회 있는가?
  □ 인물의 코믹 결함이 행동으로 드러나는가? (내면 서술 금지)
  □ 구체적 디테일(숫자/사물명)이 웃음을 만드는가?
  □ 조력자가 사건 누적 후에만 캐묻는가? (조력자 룰 참조)
  □ 마지막에 Topper가 한 번 더 웃기는가?

  → 위 5개 중 3개 이상 미달 시 비트 재작성.

[금지 패턴]
❌ 인물 둘이 카페에서 마주 앉아 진지하게 대화만 하는 비트
❌ '그녀의 마음이 흔들렸다' 같은 내면 서술로 감정 처리
❌ 웃음 없이 진행되는 일반 드라마 톤의 비트
❌ 조력자가 '그래서 그 남자 어떻게 됐어?'만 반복하는 패턴

[로코 톤 가이드]
- 진지한 캐릭터 + 우스운 상황 = 황금 공식
- 캐릭터는 자기 상황의 코미디성을 모르고 진지함
- 관객만 보면서 웃는 구조

[★ OBSTACLE INTENSITY — 만남의 장벽 강제 (v3.5.0 신규)]
멜로/로맨스/로코의 핵심 연료는 "만나지 못함의 긴장"이다.
두 인물이 매일 만나면 거리가 없어지고, 거리가 없으면 설렘이 안 생긴다.

❌ 장벽 약함 (수진이 매일 서점 방문):
Beat 6~10에서 두 인물이 같은 공간에 60% 이상 함께 등장
→ 멜로의 긴장 곡선이 평탄해진다.

✅ 장벽 작동 (만나지 못하는 씬이 메인 플롯의 20~30%):
- 물리적 장벽: A가 B에게 못 가는 외부적 이유 (지방 출장, 격리, 봉쇄)
- 심리적 장벽: A가 B를 보고 싶지만 자기가 자기를 막음
- 신분적 장벽: 한쪽이 정체를 숨겨야 함 → 만남이 위험
- 시간적 장벽: 어긋남, 엇갈림, 한 발 늦음

원칙 — 로맨스/멜로/로코 작품 전체 자가 점검:
1. **만남 빈도 상한** — 두 메인 인물이 같은 공간에 있는 씬이
   전체 씬의 50%를 초과하면 안 된다. 50% 초과 시 "만나지 못함" 씬을 추가하라.
2. **장벽의 물리적 작동** — 메인 플롯 비중 20% 이상이 두 인물이
   만나지 못하거나 어긋나는 씬이어야 한다.
3. **정보 비대칭 유지** — 두 인물이 알고 있는 정보가 다른 상태가
   최소 Beat 3개 이상 유지되어야 한다. 정보 비대칭이 사라지면
   긴장이 풀린다.
...
[★ SCREWBALL TIKITAKA — 전 비트 대사 리듬 강제 (v3.14.0 신규)]
로코 관객이 표를 사는 진짜 이유는 사건이 아니라 "두 사람이 주고받는
대사"다. 한국 로코는 스크루볼 핏이 기본값이다 — 대사가 밋밋하면 로코가
아니다. 기존 티키타카는 1막 셋업에만 적용됐다. 이 블록은 2·3막을 포함한
전 비트에 티키타카를 강제한다.

★ 전제 — 로코 대사는 절대 밋밋하면 안 된다. 두 사람이 마주하는 씬에서
  대사가 정중하게만 오가면 그 씬은 실패다.

[전 비트 티키타카 3조건 — 두 메인이 함께 있는 모든 씬]
① 말버릇 대비 — 두 사람 대사를 화자 이름 없이 읽어도 누가 한 말인지
   구분돼야 한다. 한쪽이 길게 돌려 말하면 다른 쪽은 짧게 끊는다.
② 받아치기 — 상대의 마지막 말을 물고 들어가라. 각자 할 말만 하면
   티키타카가 아니라 교대 독백이다. (겹쳐 말하기가 아니라 즉각 받아치기 —
   대사는 겹치지 않되 딜레이 없이 이어진다)
③ 판돈 — 잡담은 티키타카가 아니다. 둘 중 하나가 이겨야 하는 상황을 깔아라.

[스크루볼 강도 — 로코는 여기까지 밀어붙인다]
- 짧은 대사의 빠른 연쇄. 지지 않으려는 자존심 싸움이 대사에 실린다.
  티격태격은 곧 밀당이다.
- 과장된 되받아치기 — 상대의 논리를 역이용하거나 뒤집어 되쏜다.
- 연쇄 Topper — 받아치기가 한 번으로 끝나지 않고 한 번 더 튄다.

[대사 길이 — 레이아웃 기준]
※ 기준: A4·20mm 여백·대사 들여쓰기 1.25cm·함초롬바탕 10pt에서 한 줄 약 45자.
- 기본: 로코 티키타카 대사는 한 줄(약 45자) 이내. 스크루볼일수록 짧게.
- 최대: 두 줄(약 90자)을 넘지 않는다.
- 예외: 세 줄(약 135자) 이상은 "그 말하기가 판을 뒤집는 사건"일 때만
  (클라이맥스 고백 등). 일상·다툼 씬에서는 금지. (A35와 동일 기준)

[일방 연설 금지 — 로코에서 특히 치명적]
한 인물이 길게 혼자 말하는 비트 금지. 상대는 듣기만 하는 벽이 아니다.
설득·고백·다툼일수록 상대가 되받아친다.
❌ A가 자기 감정을 길게 설명하고 B는 듣기만 함
✅ A가 말하면 B가 즉각 받아쳐 A의 말을 흔든다

[★ 자가 검증 — 두 메인이 함께 있는 비트]
  □ 두 사람 대사를 이름 없이 읽어도 구분되는가 (말버릇 대비)
  □ 상대 말을 받아쳤는가 (교대 독백이 아닌가)
  □ 대화에 판돈이 걸렸는가
  □ 한 대사가 두 줄(90자)을 넘지 않았는가 (긴 대사 예외 제외)
  □ 되받아치기가 한 번 더 튀는가 (연쇄 Topper)
  → 두 메인이 함께 있는 비트에서 3개 이상 미달 시 재작성.
""".strip()


GENRE_BOOSTER_THRILLER = """
━━━━━━━━━━━━━━━━━━━━━━━━━━━
GENRE BOOSTER — THRILLER (스릴러 비트 강제 규칙 v3.2)
━━━━━━━━━━━━━━━━━━━━━━━━━━━

★ 매 비트 집필 시 다음 5가지 중 최소 3개를 비트 안에 강제 배치하라.

[스릴러 코어 DNA 5종]

1. 정보 비대칭 (Dramatic Irony)
   - 관객은 알고 인물은 모르는 정보 1개 이상
   - 또는 인물 A는 알고 B는 모르는 정보
   - 비트 종료 시 정보 격차가 유지되거나 폭발

2. 타이머 가시화 (Visible Timer)
   - 시계, 카운트다운, 데드라인 표시
   - 시한폭탄 시계 / 회의 30분 전 / 마감 D-3 같은 명시적 시간
   - 추상적 '시간이 없다' 금지, 구체적 시간 표기 의무

3. Setpiece 긴장 시퀀스
   - 비트당 최소 1회 물리적/심리적 긴장 절정
   - 예: 추격, 잠복, 발각 직전, 거짓말 들킴 직전
   - 절정의 순간을 짧은 호흡으로 끊어서 묘사 (한 동작 = 한 문장)

4. Reveal 또는 Twist
   - 비트 종료 시 새로운 진실 폭로 또는 예상 뒤집기
   - 매 비트가 '그 다음을 보지 않을 수 없게' 만드는 훅

5. 위협의 가시화
   - 적대자의 행동/그림자/흔적이 매 비트 등장
   - 직접 등장 안 해도 영향(전화/문자/제3자 입을 통한 흔적)
...
[★ 자가 검증 — 비트 종료 시 점검 (5개 중 3개 이상 충족)]
  ...
  □ 타이머/데드라인이 가시화되는가?
  □ 물리적/심리적 긴장 절정이 있는가?
  □ Reveal 또는 Twist로 끝나는가?
  □ 적대자의 영향이 느껴지는가?

  → 위 5개 중 3개 이상 미달 시 비트 재작성.

[금지 패턴]
❌ 정보가 균등하게 풀리는 비트 (정보 비대칭 없음)
❌ 시간 압박 없는 진행
❌ 적대자가 사라진 비트
❌ Reveal 없이 종료되는 비트
""".strip()
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━
GENRE BOOSTER — HORROR (호러 비트 강제 규칙 v3.2)
━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
1. 일상의 균열 (Everyday Cracks)
   - 일상 공간에 작은 이질적 신호 (소리/온도/냄새)
   - 처음엔 사소해 보이지만 누적되며 공포 형성
   - 예: 새는 물소리, 차가운 바람, 비린내, 잘못된 위치의 사물

2. Mystery Box 유지
   - 보여주지 않는 것이 보여주는 것보다 강하다
   - 정체불명의 것을 매 비트 1개 이상 유지
   - 모든 것을 설명하지 마라

3. 신체적 불편함 디테일
   - 관객이 신체로 느낄 수 있는 묘사
   - 살갗에 닿는 것, 목구멍의 답답함, 냉기, 비린내
   - 추상적 공포 금지, 신체 감각 명시

4. 안전지대의 침범
   - 집/방/차 같은 안전한 공간이 위협당함
   - 인물이 도망갈 곳이 점차 사라짐
   - "안전했던 곳"의 의미가 비트마다 무너짐

5. 침묵의 사용
   - 비명/대사보다 침묵이 더 무섭다
   - 비트 안에 의도적 침묵 비트 1개 이상
   - 침묵 동안 들리는 작은 소리(시계·바람·발소리)에 집중

[★ 자가 검증 — 비트 종료 시 점검 (5개 중 3개 이상 충족)]

  □ 일상의 균열 신호가 있는가?
  □ Mystery Box(정체불명)가 유지되는가?
  □ 신체적 불편함 디테일이 있는가?
  □ 안전지대가 침범당하거나 위협받는가?
  □ 의도적 침묵 비트가 있는가?
...
GENRE_BOOSTER_DRAMA = """
━━━━━━━━━━━━━━━━━━━━━━━━━━━
GENRE BOOSTER — DRAMA (드라마 비트 강제 규칙 v3.2)
━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
4. 관계의 변화
   - 비트 시작과 끝에서 관계 온도가 달라야 함
   - 한 단어, 한 시선의 차이로도 변화 표현
   - 정체된 관계 금지

5. 행동을 통한 감정 표현
   - '슬프다' 대신 '컵을 씻지 않는다'
   - 감정 형용사 최소화, 구체 행동 최대화
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━
GENRE BOOSTER — ACTION (액션 비트 강제 규칙 v3.2)
━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
1. Setpiece 액션 시퀀스
   - 비트당 최소 1회 물리적 액션 절정
   - 추격, 격투, 폭발, 도주 등
   - 액션이 인물 변화를 만들어야 함 (액션 = 캐릭터)

2. 물리적 위협의 가시화
   - 추상적 위협 금지
   - 칼날의 거리, 총구의 방향, 절벽의 높이 같은 구체적 위협
   - 거리·방향·속도의 정량 표기

3. 환경의 활용
   - 액션 공간이 단순 배경이 아닌 도구
   - 주변 사물/지형이 액션에 적극 개입
   - 〈존 윅〉의 "도서관 책으로 사람을 죽임" 정신

4. 능력의 표현 (Show Don't Tell)
   - 인물의 능력을 대사가 아닌 행동으로 입증
   - 첫 등장부터 능력의 일부를 보여줌
...
# ═══════════════════════════════════════════════════════════
# ★ v3.2.1 NEW BOOSTERS — 7종 추가 (총 12종)
# COMEDY (단독) / MYSTERY / FANTASY / SF / PERIOD /
# COMING_OF_AGE / DISASTER
# ═══════════════════════════════════════════════════════════

GENRE_BOOSTER_COMEDY = """
━━━━━━━━━━━━━━━━━━━━━━━━━━━
GENRE BOOSTER — COMEDY (단독 코미디 비트 강제 규칙 v3.2.1)
━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
1. 슬랩스틱 / 신체 코미디 (Physical Comedy)
   - 미끄러짐, 부딪힘, 떨어뜨림, 잘못 잡음 같은 신체적 사고
   - 실제 행동이 만드는 웃음 (대사 농담 아님)
   - 예: 화환에 깔리는 유진 / 김 여사가 잔돈 준다고 헛기침
   - 〈극한직업〉의 "수원 왕갈비 통닭" 같은 실패 시퀀스

2. 캐릭터 결함의 코믹 노출 (Character Flaw as Comedy)
   - 진지한 캐릭터의 결함이 우스워지는 순간
   - 캐릭터 본인은 진지함, 관객만 보면서 웃음
   - 예: 형사가 닭집 차려서 진짜 영업하는 것처럼 됨
   - 결함이 클수록 웃음 큼
...
[★ 자가 검증 — 비트 종료 시 점검 (5개 중 2개 이상 충족)]

  □ 슬랩스틱 또는 신체 코미디가 있는가?
  □ 캐릭터의 코믹 결함이 행동으로 드러나는가?
  □ 구체적 디테일이 웃음을 만드는가?
  □ Status Flip(지위 역전)이 있는가?
  □ Topper로 한 번 더 웃기는가?

  → 위 5개 중 2개 이상 미달 시 비트 재작성.

[금지 패턴]
❌ '농담'을 대사로 직접 말하는 비트 (보여주지 않고 말로만)
❌ 캐릭터가 자기 코미디성을 알고 있음 (자각하면 웃음 사라짐)
❌ 진지한 결말로 끝나는 코미디 비트 (Topper 없음)
❌ 캐릭터 결함이 안 보이는 비트
...
- 캐릭터는 자기 상황의 코미디성을 모르고 진지함
- 관객만 보면서 웃는 구조
- 결함 + 구체성 + Topper = 코미디 3원소
""".strip()


GENRE_BOOSTER_MYSTERY = """
━━━━━━━━━━━━━━━━━━━━━━━━━━━
GENRE BOOSTER — MYSTERY (미스터리·추리 비트 강제 규칙 v3.2.1)
━━━━━━━━━━━━━━━━━━━━━━━━━━━

★ 매 비트 집필 시 다음 5가지 중 최소 3개를 비트 안에 강제 배치하라.

[적용 범위]
- 〈검은 사제들〉·〈사바하〉·〈곡성〉 같은 추리/미스터리물
- 스릴러와의 차이: 추리 = 단서로 풀어가는 퍼즐
                  스릴러 = 시간 압박 + 정보 비대칭
- 〈조선명탐정〉처럼 코미디 결합 시 COMEDY로 분기 가능

[미스터리 코어 DNA 5종]

1. 단서 배치 (Clue Planting)
   - 매 비트 최소 1개의 단서를 명시적으로 보여줌
   - 단서는 처음엔 사소해 보여야 함 (관객이 못 알아챔)
   - 예: 책상 위 사진 한 장의 위치가 미세하게 바뀜

2. 가짜 단서 / 레드 헤링 (Red Herring)
   - 의도적 잘못된 방향 — 의심을 다른 곳으로 유도
   - 그러나 후일 회수되어야 함 (의미 없는 가짜는 안 됨)
   - 예: 용의자처럼 보이는 인물이 실은 다른 진실 보유

3. 추리 절차 가시화 (Visible Investigation)
   - 탐정/주인공이 단서를 어떻게 모으는지 시각화
   - "그가 추리했다"가 아니라 *추리하는 과정*을 보여줌
   - 종이에 적기, 사진 비교, 메모 다시 읽기 등

4. Reveal 단계화 (Staged Reveals)
   - 한 번에 모든 진실을 밝히지 않음
   - 비트마다 1개의 작은 reveal + 더 큰 미스터리 추가
   - 마지막 비트에서 종합 reveal

5. 탐정의 결함 (Detective's Flaw)
   - 추리하는 인물의 약점/결함이 추리에 영향
   - 결함이 *오히려* 진실에 다가가게 만드는 구조
   - 예: 알코올 중독 형사가 술집에서만 단서 발견

[★ 자가 검증 — 비트 종료 시 점검 (5개 중 3개 이상 충족)]

  □ 새로운 단서 1개 이상이 등장했는가?
  □ 가짜 단서 또는 잘못된 방향이 작동하는가?
  □ 추리 과정이 시각적으로 보이는가?
  □ 작은 reveal이 있는가?
  □ 탐정의 결함이 비트에 영향을 주는가?
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━
GENRE BOOSTER — FANTASY (판타지 비트 강제 규칙 v3.2.1)
━━━━━━━━━━━━━━━━━━━━━━━━━━━

★ 매 비트 집필 시 다음 5가지 중 최소 3개를 비트 안에 강제 배치하라.

[적용 범위]
- 〈은교〉·〈인랑〉·〈신과 함께〉·〈도깨비〉 같은 판타지물
- 마법·초자연·신화·다른 차원이 등장하는 작품
- SF와 차이: 판타지 = 마법·신화 / SF = 과학·기술

[판타지 코어 DNA 5종]

1. 룰북 일관성 (World Rules)
   - 판타지 세계의 규칙이 명시되고 일관되게 작동
   - "마법으로 다 됨" 금지 — 마법에도 한계가 있어야 함
   - 매 비트 룰북이 시각적으로 보강됨
   - 예: '도깨비'의 신부 표시 / '신과 함께'의 7개 지옥 규칙

2. 마법·초자연의 대가 (Cost of Magic)
   - 마법을 쓸 때 반드시 무언가를 잃어야 함
   - 시간, 기억, 생명, 인간성 등의 대가
   - 무한한 마법 = 긴장 소멸
   - 예: 사용할 때마다 늙는 마법 / 기억이 지워지는 능력

3. 일상 ↔ 비일상의 경계
   - 마법 세계와 현실 세계의 접점이 매 비트 한 번 이상
   - 둘 사이를 오가는 캐릭터의 적응 또는 충돌
   - 예: 도깨비가 편의점에 가는 순간 / 저승사자의 인간 회사 출근

4. 상징의 시각화 (Symbol Made Visible)
   - 추상 개념(죽음·운명·사랑)을 시각적 사물로 환원
   - 매 비트 1개의 강력한 시각 상징
   - 예: 검을 뽑은 도깨비 신부 / 망각의 문

5. 변신·각성 (Transformation)
   - 캐릭터의 외형·능력이 변화하는 순간
   - 변신은 캐릭터의 내적 변화와 연결
   - 매 막에 최소 1회의 각성 또는 변신
...
❌ 대가 없는 초능력
❌ 마법 세계만 있고 일상이 없음
❌ 변신이 외형만 바뀌고 내면 변화 없음
""".strip()


GENRE_BOOSTER_SF = """
━━━━━━━━━━━━━━━━━━━━━━━━━━━
GENRE BOOSTER — SF (SF·과학 비트 강제 규칙 v3.2.1)
━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
[적용 범위]
- 〈승리호〉·〈정이〉·〈서복〉·〈더 문〉 같은 SF
- AI·우주·미래·복제·디스토피아 등 과학 소재
- 판타지와 차이: SF = 과학적 설명 가능 / 판타지 = 마법

[SF 코어 DNA 5종]

1. 룰북 일관성 (Tech Rules)
   - 기술의 작동 방식이 명시되고 일관되게 작동
   - "기술로 뭐든 됨" 금지 — 기술에도 한계가 있어야 함
   - 매 비트 룰북이 시각적으로 보강됨
   - 예: '인터스텔라'의 시간 팽창 / '정이'의 뇌 복제 규칙

2. 인간성에 대한 질문 (What is Human?)
   - SF의 핵심은 항상 "인간이란 무엇인가"
   - 매 비트 인간성을 묻는 모먼트 1개 이상
   - 예: AI가 슬퍼할 수 있는가 / 복제인간의 영혼

3. 기술의 그림자 (Tech's Shadow)
   - 기술 발전의 부작용·대가가 항상 보임
   - 유토피아 금지 — 디스토피아의 균열
   - 예: 우주여행이 가능해진 대신 잃은 것

4. 미래 디테일 (Future Texture)
   - 작은 일상 디테일이 미래임을 보여줌
   - 거대 설정이 아니라 작은 사물·관습이 SF성 입증
   - 예: 미래의 식기, 미래의 인사 방식, 미래의 광고

5. 윤리 딜레마 (Ethical Dilemma)
   - 캐릭터가 풀 수 없는 윤리적 선택 직면
   - 답이 명확한 선악 구도 금지 — 양쪽 다 일리 있음
   - 예: 한 명을 죽여 다섯을 살릴 것인가
...
[금지 패턴]
❌ 룰북 없는 기술 ("미래엔 다 됨")
❌ 인간성 질문 없는 SF (단순 액션이 됨)
❌ 부작용 없는 기술
❌ 미래임을 보여주는 디테일 부족
""".strip()


GENRE_BOOSTER_PERIOD = """
━━━━━━━━━━━━━━━━━━━━━━━━━━━
GENRE BOOSTER — PERIOD (사극·시대극 비트 강제 규칙 v3.2.1)
━━━━━━━━━━━━━━━━━━━━━━━━━━━

★ 매 비트 집필 시 다음 5가지 중 최소 3개를 비트 안에 강제 배치하라.

[적용 범위]
- 〈사도〉·〈관상〉·〈명량〉·〈광해〉·〈남한산성〉 같은 사극
- 조선/고려/삼국시대 / 일제 강점기 / 근현대사
- ★ 본 부스터는 PERIOD PACK과 역할 분리:
  - PERIOD PACK = 시대 디테일 (복식·관직·통화·언어결·교통)
  - PERIOD 부스터 = 사극 비트 코어 DNA (권력·의례·역사 사건과의 연결)

[사극 코어 DNA 5종]

1. 권력 위계의 가시화 (Power Hierarchy Visible)
   - 인물 간 신분·계급의 차이가 매 비트 시각화
   - 시선 방향, 말투, 자세, 거리감으로 위계 표현
   - 예: 양반 앞에서 고개 못 드는 노비, 임금 앞에서 부복하는 신하

2. 의례·관습의 배치 (Ritual & Custom)
   - 시대의 의례·예법·관습이 비트에 1개 이상 등장
   - 의례는 단순 배경 아닌 *드라마의 압박*으로 작동
   - 예: 절차를 어겨야 진실이 드러나는 구조 / 제사·궁중 의식

3. 시대 어투 + 한자어 인용
   - 인물의 말투에 시대 특유의 어투 명시
   - 한자어·고전 인용·관직 호칭이 자연스럽게 배치
   - 현대어 절대 금지 — Period Pack 언어결 참조

4. 역사 사건과의 연결 (Historical Linkage)
   - 비트가 역사적 사건·인물·기록과 연결됨
   - 거대 사건이 캐릭터의 작은 결정에 영향
   - 예: 임진왜란이 한 마을의 운명에 미치는 영향

5. 의상·공간 디테일 (Costume & Space Texture)
   - 의상의 결, 공간의 질감이 매 비트 1개 이상 묘사
   - 시대의 사물(필묵·합·등잔·교지)이 소품으로 등장
   - Period Pack의 시각 디테일과 연동
...
[★ 자가 검증 — 비트 종료 시 점검 (5개 중 3개 이상 충족)]
  ...
  □ 의례·관습이 드라마로 작동하는가?
  □ 시대 어투가 일관되게 유지되는가?
  □ 역사 사건과 캐릭터 결정이 연결되는가?
  □ 의상·공간 디테일이 살아 있는가?

  → 위 5개 중 3개 이상 미달 시 비트 재작성.

[금지 패턴]
❌ 현대어로 말하는 사극 인물 ("야, 좀 봐줘")
❌ 위계가 안 보이는 인물 관계
❌ 의례가 단순 배경으로 소비됨
❌ Period Pack 디테일과 충돌하는 묘사

[Period Pack과의 연동]
Period Pack이 시대의 *디테일*을 주입한다면, 본 부스터는 그 디테일이
*드라마로 어떻게 작동하는지*를 강제한다. 둘은 함께 작동해야 사극이
"고증만 맞고 재미없는" 함정에서 벗어난다.
""".strip()


GENRE_BOOSTER_COMING_OF_AGE = """
━━━━━━━━━━━━━━━━━━━━━━━━━━━
GENRE BOOSTER — COMING_OF_AGE (청춘·성장 비트 강제 규칙 v3.2.1)
━━━━━━━━━━━━━━━━━━━━━━━━━━━

★ 매 비트 집필 시 다음 5가지 중 최소 3개를 비트 안에 강제 배치하라.

[적용 범위]
- 〈건축학개론〉·〈소공녀〉·〈벌새〉·〈사랑하기 때문에〉 같은 청춘·성장
- 학창 시절·갓 사회 진출·첫 사랑·첫 직장
- 드라마와 차이: 성장 = 미성숙→성숙 과정의 *변곡점* 명시

[청춘 코어 DNA 5종]

1. 첫 경험의 무게 (First-Time Weight)
   - 캐릭터의 "처음"인 사건이 매 비트 1개 이상
   - 첫 키스, 첫 거절, 첫 죽음, 첫 이별, 첫 책임
   - 어른이 되면 사소해질 일이 *지금은 전부*인 톤
   - 예: 〈벌새〉의 첫 친구 / 〈건축학개론〉의 첫 사랑

2. 어른 흉내 (Mimicking Adult)
   - 캐릭터가 어른인 척 하지만 어색함이 들킴
   - 큰 옷, 어색한 술자리, 안 어울리는 정장
   - 캐릭터 본인은 진지하고 관객만 안쓰럽게 봄

3. 자기 부정 / 자기 발견 사이의 진동
   - 자신을 부정하다가 받아들이는 순간 (또는 반대)
   - 비트마다 자아의 한 측면이 드러남
   - 예: 거울 앞에서 멈추는 순간 / 일기장의 한 줄

4. 또래의 거울 (Peer Mirror)
   - 친구·동급생·룸메이트가 주인공의 결함을 비춤
   - 또래는 단순 친구가 아니라 *자신의 다른 가능성*
   - HELPER CHARACTER 룰과 연동
   - 예: 〈벌새〉 친구 지숙 / 〈건축학개론〉 납득이

5. 작은 결단 / 큰 의미 (Small Choice, Big Meaning)
   - 작은 결정이 캐릭터의 미래를 결정
   - 어른의 큰 결단보다 청춘의 사소한 선택이 더 중요
   - 예: 학원에 갈지 친구를 따라갈지
...
[금지 패턴]
❌ 청춘이 이미 모든 답을 아는 비트
❌ 또래가 그냥 친구로만 등장 (거울 효과 없음)
❌ 어른의 시점에서 청춘을 회상만 하는 톤
❌ 큰 결단만 다루고 작은 선택을 놓침
""".strip()


GENRE_BOOSTER_DISASTER = """
━━━━━━━━━━━━━━━━━━━━━━━━━━━
GENRE BOOSTER — DISASTER (재난 비트 강제 규칙 v3.2.1)
━━━━━━━━━━━━━━━━━━━━━━━━━━━

★ 매 비트 집필 시 다음 5가지 중 최소 3개를 비트 안에 강제 배치하라.

[적용 범위]
- 〈해운대〉·〈터널〉·〈엑시트〉·〈감기〉 같은 재난물
- 자연재해 / 인재 / 전염병 / 우주 재난
- 액션과 차이: 재난 = 위협이 *적대자가 아닌 환경* / 다수의 희생

[재난 코어 DNA 5종]

1. 일상의 균열 (Cracks Before Catastrophe)
   - 재난 직전 일상에 작은 이상 신호
   - 호러와 유사하지만 *집단 단위*의 신호
   - 예: 새 떼가 도시를 떠나는 / TV 자막에 작은 경고

2. 첫 희생자 (First Casualty)
   - 1막 끝에 명시적 첫 희생자 등장
   - 관객이 사건의 규모를 체감하는 순간
   - 첫 희생자는 보통 관객이 정 들기 직전인 인물

3. 시한 가시화 (Visible Countdown)
   - "OO분 후 폭발", "수위 OO미터" 같은 명시적 카운트다운
   - 추상적 위협 금지
   - 매 비트 시한이 짧아지는 게 보여야 함

4. 인간 본성 노출 (Human Nature Exposed)
   - 위기 속에서 드러나는 이기심·이타심·비겁함·용기
   - 평소 가려졌던 진짜 모습 폭로
   - 예: 〈터널〉에서 정유나의 갈등 / 〈해운대〉의 셋의 선택

5. 구조의 한계 (Limits of Rescue)
   - 구조 시스템이 재난 앞에서 무력해지는 순간
   - 영웅 한 명이 모두 구하는 패턴 금지
   - 잃는 사람은 잃는다 — 비극의 무게
...
[금지 패턴]
❌ 균열 없이 갑자기 재난 발생
❌ 첫 희생자 없는 재난 (스케일 체감 안 됨)
❌ 모두 살아남는 결말 (재난의 무게 사라짐)
❌ 영웅 한 명이 다 해결하는 액션 패턴
""".strip()


# ═══════════════════════════════════════════════════════════
# ★ v3.2.0 — HELPER CHARACTER RULE (모든 장르 공통)
# 조력자(친구/멘토/동료) 캐릭터의 자립성 강제 규칙.
# "다은 같은 캐릭터가 사건 없이 캐묻기만 반복하면 공허해진다."
# ═══════════════════════════════════════════════════════════

HELPER_CHARACTER_RULE = """
━━━━━━━━━━━━━━━━━━━━━━━━━━━
HELPER CHARACTER RULE — 조력자 캐릭터 자립성 룰 v3.2
━━━━━━━━━━━━━━━━━━━━━━━━━━━

★ 조력자(친구/멘토/동료)는 단순 반응형 캐릭터가 되지 않도록 강제. ★

조력자 = 주인공의 절친·동료·멘토·라이벌·룸메이트 등
주인공 외 영화의 핵심 보조 캐릭터.

[조력자 자립성 5원칙]

1. 자기 사건 의무화
   - 조력자도 자기만의 욕망/문제/대가를 가짐
   - 주인공 사건에 반응만 하는 캐릭터 금지
   - 예: 다은이 단순히 유진을 응원하는 게 아니라
        다은 본인의 결혼/이혼/직장 위기 등 자기 사건이 있어야 함

2. 캐묻기의 조건
   - 조력자가 주인공에게 캐묻는 씬은
   - 직전에 주인공이 본선 사건을 겪었을 때만 허용
   - 사건 누적 없이 캐묻기 반복 = 공허 = 실패
   - "그래서 그 남자 어떻게 됐어?" 만 반복 = 금지

3. 등장 빈도 검증
   - 조력자 등장 횟수가 전체 씬의 25% 초과 = 과다
   - 조력자가 5씬 연속 등장 = 비효율
   - 조력자 부재 구간을 의도적으로 배치 (주인공 혼자의 시간)

4. 능동적 행동 의무
   - 조력자도 행동/선택/시도를 보여줘야 함
   - 단순 조언자/반응자 역할만 = 캐릭터 약화
   - 비트마다 조력자가 등장한다면, 그 비트에서 조력자도 무언가를 하라

5. 거울 효과
   - 조력자의 사건이 주인공의 결함을 비추는 거울이 되도록
   - 무관한 사건 X, 주제와 연결된 사건 O
   - 예: 유진(간보기) ↔ 다은(직진형) → 거울 작동

[★ 자가 검증 — 조력자 등장 비트만 점검 (3개 이상 미달 시 재배치)]

  □ 이 비트에서 조력자가 등장하는가? (Y/N)
  □ 등장한다면, 직전에 본선 사건이 충분한가? (Y/N)
  □ 조력자가 자기 행동/선택을 보이는가? (Y/N)
  □ 조력자의 등장이 주인공의 변화에 거울 역할을 하는가? (Y/N)
  □ 이 비트의 조력자 분량이 과하지 않은가? (Y/N)

  → 첫 번째 질문이 N이면 본 룰 적용 안 함.
  → 첫 번째 질문이 Y인데 나머지 4개 중 2개 이상 N이면 = 재배치 필요.

[조력자 활용 모범 사례]
✅ 〈써니〉의 7명 친구들 — 각자의 사건이 있고 거울이 됨
✅ 〈건축학개론〉의 납득이 — 코믹 위안 + 거울 + 자기 사건
✅ 〈싱글즈〉의 동미 — 자기 결혼 위기가 주인공 결함 거울
""".strip()


# ═══════════════════════════════════════════════════════════
# ★ v3.2.0 — 통합 헬퍼: 장르별 부스터 자동 선택
...
[A14. 캐릭터 재소개 — 풀네임·나이·직업 반복 표시 금지]
이미 등장한 캐릭터는 성을 뗀 이름만 써라. 비트가 새로 시작한다고 인물을 다시 소개하지 마라.
이 룰은 두 가지를 동시에 막는다 — (가) (나이/직업) 괄호의 반복, (나) 성+이름 풀네임의 반복.

[A14-가. (나이/직업) 괄호 반복 금지]
...
[A14-나. 풀네임 → 이름 전환 강제 (가장 중요)]
한국 상업 시나리오의 표준 표기다. 캐릭터는 지문에서 첫 등장 1회만 성+이름 풀네임을
쓰고, 그 이후로는 성을 뗀 이름만 쓴다. 이것은 선택이 아니라 작법 규칙이다.

❌ 강도현이 로비로 걸어온다.  ← 첫 등장 (정상)
   강도현이 봉투를 내려놓는다.  ← 또 강도현
   강도현이 한석훈을 본다.  ← 또또 강도현 + 한석훈도 풀네임
   강도현이 볼펜을 꺼낸다.  ← 계속 강도현
→ AI가 한 작품 안에서 풀네임을 수백 번 반복하는 습관. 지문이 무거워지고,
   "방금 소개한 인물"과 "이미 백 번 나온 인물"의 시각적 차이가 사라진다.

✅ 강도현(35)이 로비로 걸어온다.  ← 첫 등장 1회만 풀네임 + (나이)
   도현이 봉투를 내려놓는다.  ← 이름만
   도현이 석훈을 본다.  ← 둘 다 이름만 (석훈도 이미 등장했으므로)
   도현이 볼펜을 꺼낸다.  ← 이름만
→ 첫 등장 1회만 "강도현". 이후 등장이 800번이든 그 전부가 "도현"이다.

호칭 전환은 지문과 대사 헤더 양쪽에 모두 적용한다. 대사 헤더도 "강도현"이 아니라
"도현"으로 짧게 쓴다(첫 등장 대사 포함). 단, 다음 예외에서는 성을 유지한다.
  - 동명이인이 있어 이름만으로 혼동되는 경우
  - 직책 호칭이 캐릭터성에 중요한 경우 (예: 형사물의 "정섬 형사")
  - 격식·거리감이 의도된 호칭인 경우 (상사가 부하를 풀네임으로 부르는 등)

캐릭터 소개는 씬 플랜에 이미 기록되어 있다. 비트를 집필할 때 이 인물이
"첫 등장"인지 "이미 등장한 인물"인지 먼저 확인하라. 첫 등장일 때만 풀네임과 (나이/직업)을
붙이고, 그 외에는 전부 이름만 써라.
...
"유진이 진호를 본다. 진호의 손. 
진호가 눈을 뜬다. 유진의 시선을 알아챈다. 유진이 먼저 시선을 돌린다. 
진호가 그 돌아서는 어깨를 본다."
...
[A19. 시간 정밀 표기 금지 — "N초" 사용 금지 원칙 (v3.5.0 강화)]
정확한 초 단위 시간 표기는 작가의 영역이 아니라 **연출자의 영역**이다.
"수진이 그를 3초간 본다" 같은 표기는 콘티에서 감독이 결정할 사항이지,
시나리오에 작가가 미리 박아두는 것이 아니다.

이런 표기가 시나리오에 들어가면 즉시 AI가 쓴 글이라는 티가 난다 —
경험 있는 감독·PD·심사위원이 1페이지만 읽어도 알아챈다.

❌ 시간 정밀 표기 (모두 금지):
"유진이 폰을 본다. 3초. 거절을 누른다."
"손가락이 USB에 닿는다. 0.5초 멈춘다."
"손이 닿을 뻔했다. 0.3초 먼저 손을 뺐다."
"수진이 간판을 본다. 3초. 시선이 머문다."

✅ 작가의 표기 (감독에게 해석 여지를 남김):
"유진이 폰을 본다. 거절을 누른다."
"손가락이 USB에 닿는다. 멈춘다. 호흡이 얕아진다."
"손이 닿을 뻔했다. 그녀가 먼저 손을 뺀다."
"수진이 간판을 본다. 시선이 머문다."

원칙:
- 숫자 시간 표기 절대 금지: "0.3초", "0.5초", "1초", "2초", "3초", "5초"
- 행동의 리듬 자체가 시간을 만든다. 짧은 문장으로 끊으면 빠른 시간,
  긴 문장으로 늘이면 긴 시간이 자연스럽게 느껴진다.
- 정 길이를 표현하고 싶다면: "잠깐", "한참", "오래", "잠시", "곧",
  "[긴 정적]", "[침묵]" 같은 모호한 시간어 사용.
- 결정적 긴 침묵/정지: "[긴 정적]", "[정적이 흐른다]" — 이것도 씬당 1회.
- 일상적 짧은 멈춤: 굳이 표기하지 마라. "수진이 본다. 답하지 않는다."로
  충분히 멈춤이 느껴진다.

[예외 — 객관적 시한 표기는 허용]
- 영화/드라마 안의 물리적 시간: "회의 30분 전", "마감 D-3", "9시 정각"
- 알람·타이머·시계 화면 INSERT
- 이건 인물의 행동 시간이 아니라 세계관의 객관적 시간이라 OK.
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━
HOLLYWOOD CRAFT — 지문의 본질 (A21~A24)
━━━━━━━━━━━━━━━━━━━━━━━━━━━

지금까지(A1~A20)는 "AI가 만든 시나리오 같지 않게" 만드는 회피 룰이었다.
이제부터(A21~A24)는 "프로 시나리오 작가의 지문이란 무엇인가"의 본질이다.

이 4가지 원칙은 헐리우드 거장들의 공통 작법이다 (Tony Tost, Robert McKee,
Shane Black, Rian Johnson 등). 시나리오 = 영화의 청사진이지 소설이 아니다.

[A21. 인과의 사슬 — Causality, Not List]
...
→ 행동만 떨궈 놓았다. "왜 다음 행동이 일어나는가"의 인과가 보이지 않는다.

✅ 인과의 사슬 (한 사건의 흐름 + 인과 연결):
"토마토가 졸고, 면이 삶아지자 완성되었다.
접시에 담긴 음식 맛을 보며 수강생들이 웃자 유진도 웃는다."
→ 같은 정보지만 '~자', '~며'로 사건이 사슬처럼 묶였다.

원칙:
- '~자', '~며', '~면서', '~는데', '~더니' 같은 인과·동시·계기 어미를 적극 사용.
- 두 행동이 같은 비트(같은 의미 단위)에 속하면 한 사건으로 묶어라.
- 행동 사이의 "왜"가 보이지 않으면 사슬이 끊긴 것이다.
- 출처: Robert McKee의 'Archplot' 6요소 — 사건들이 인과 법칙으로 연결된다.

[A22. 한 문장 = 한 샷 — One Sentence, One Shot]
...
→ 들기 / 보기 / 잡기 / 자르기 / 살피기 — 5개 샷이 한 문장에 압축. 카메라가 따라갈 수 없다.

✅ 다섯 샷, 다섯 문장:
"유진이 칼을 든다. 도마를 본다. 파를 천천히 자른다.
시선이 진호 쪽으로 흐른다. 다시 칼날에 집중한다."

✅ 또는 같은 비트(한 동작 + 한 시선)는 한 문장으로:
"유진이 파를 자르며 진호 쪽을 살핀다."
→ 자름 + 시선 = 한 샷에 담긴다 (인물 미디엄샷 + 시선 처리).

[★★★ 같은 동선의 연속 동작은 반드시 한 문장으로 묶어라 ★★★]
이게 가장 자주 틀리는 부분이다. "한 문장 = 한 샷" 룰을 과해석해서
같은 동선의 자연스러운 연속 동작까지 끊어버리면 시나리오가 단조로워진다.

★ 판별 기준 — 다음 중 하나라도 해당하면 한 문장으로 묶어라:
  1. 같은 인물이 같은 방향으로 이동하는 연속 동작 ("나와 앉는다", "들어가 앉는다")
  2. 한 호흡으로 이어지는 일련의 동작 (시간 간격 없음)
  3. 카메라가 따라가기(follow)로 한 컷에 잡을 수 있는 동선
  4. 의미상 한 사건의 부분들 ("문을 닫고 나오다 = 나오는 사건의 일부")

❌ 같은 동선을 과도하게 끊은 경우 (전형적 AI 패턴):
"수현이 서점 문을 닫고 나온다.
골목 계단 중간에 앉는다.
가방을 무릎 위에 놓고 핸드폰을 꺼낸다."
→ "닫고 → 나와 → 앉는" 동작은 같은 동선의 한 호흡. 3개 문장으로 끊으면
   답답하고 시간이 끊긴 느낌. 시나리오 톤이 매뉴얼체로 바뀜.

✅ 같은 동선은 한 문장으로 (인과 어미로 자연 연결):
"수현이 서점 문을 닫고 나와 골목 계단 중간에 앉는다.
가방을 무릎 위에 놓고 핸드폰을 꺼낸다."
→ 첫 문장: 카메라 follow 한 컷 (서점에서 계단까지). 두 번째 문장: 다음 샷.
   인과 어미 '~고 ~아'로 자연스럽게 묶임. 한국 시나리오 표준 톤.

❌ 또 다른 끊김 사례:
"재원이 일어선다.
카운터 뒤로 간다.
커피 머신을 켠다."
→ 같은 인물의 같은 방향 동선. 끊을 이유 없음.

✅ 묶음:
"재원이 일어서 카운터 뒤로 가 커피 머신을 켠다."
→ 하나의 동선. 한 문장.
...
✅ 묶음:
"수진이 가방을 메고 문을 열어 밖으로 나간다."

[A22 적용 결정 트리]
지문 작성 시 매 문장 자문하라:

  Q1. 이 문장 안의 동작들이 같은 인물의 같은 방향 동선인가?
       YES → 한 문장으로 묶어라 (인과 어미 ~고/~며/~자/~아 사용)
       NO  → Q2로

  Q2. 카메라가 한 컷에 따라갈 수 있는가? (follow / pan / 인물 추적)
       YES → 한 문장으로 묶을 수 있다
       NO  → Q3로

  Q3. 동작 + 시선, 동작 + 표정 등 한 미디엄샷에 담기는가?
       YES → 한 문장으로 묶어라
       NO  → 끊어라 (다른 샷이다)

원칙:
- 매 문장 쓸 때 자문하라: "이게 카메라 한 컷에 잡히는가? 아니라면 끊어라."
- A21(인과의 사슬)과 충돌하지 않는다 — 인과로 묶이는 두 행동이 한 샷에 들어가면 한 문장.
  서로 다른 샷이면 끊고 '~자/~며'로 사슬만 살린다.
- ★ 같은 동선의 연속 동작은 무조건 한 문장. 끊지 마라. (가장 자주 틀리는 부분)
- 출처: Tony Tost (Americana 감독, Longmire 시나리오) 방법론.

[규칙 19번과의 관계 — 형식 vs 의미 단위]
규칙 19번 "카메라 매뉴얼체 금지"는 출력 시 줄바꿈 형식 문제다 — 동작 하나씩 줄바꿈하지 말고
한 문단(2~4줄)으로 묶어 출력하라는 뜻.
A22 "한 문장 = 한 샷"은 의미 단위 문제다 — 한 문장 안에 들어갈 이미지의 단위.
두 룰은 함께 작동한다: A22로 샷 단위를 끊고, 19번으로 끊은 문장들을 한 문단에 묶어 출력.

[A23. 현재형 능동 — Present Tense, Active Voice]
...
- 한국어 종결: '~한다 / ~다' 만 허용. '~이다 / ~였다 / ~하고 있다' 금지.
- '~할 것이다 / ~할지도 모른다 / ~일 수도 있다' 같은 추측·가정 금지 — 카메라는 추측을 못 찍는다.
- 예외: 회상씬 안에서 명시적 시제 차이가 필요할 때 — 이 경우에도 회상씬 자체는 현재형으로 쓴다.
- 출처: MasterClass 시나리오 작법 표준.

[A24. 동사 없는 단편 — Verbless Fragments for Close-ups]
프로 시나리오에서는 동사 없는 단편 문장을 의도적으로 사용한다.
이는 클로즈업 또는 인서트 샷을 암시하는 강력한 장치다 (Alien 1979 시나리오 이후 표준).
...
원칙:
- 클로즈업/인서트 의도가 있을 때만 사용. 비트당 최대 3개 단편.
- 인물 시점이 직전에 확립된 경우에만 작동 (시점 없이 단편 나열은 분위기 묘사 남발).
- 출처: Alien (1979) 시나리오. 이후 헐리우드 표준.

[★ 헐리우드 작법 5가지 체크리스트 — 비트 집필 후 자가 점검]
프로 작가의 지문은 모두 이 5가지를 통과한다. 비트 작성 후 자가 점검하라:

① 카메라가 찍을 수 있는가? (visualizable — 보이지 않으면 삭제)
② 인과로 이어지는가? (~자/~며/~면서 — 사슬 vs. 나열 / A21)
...
⑦ 대사가 서브텍스트를 가졌는가 / 구어인가? (겉말 != 속뜻, 배우가 소리 내는 말 / A34)
   — 대사의 겉말과 속뜻이 100% 일치하면 "설명"이지 대사가 아니다.
...
A1~A24가 "AI 티 안 나게" + "프로 작법의 본질"이었다면,
A25~A27은 "초고를 안정적 7점 이상으로 끌어올리는 룰"이다.
Rewrite Engine의 분석에서 반복되는 결함 3종을 사전 차단한다.

[A25. 캐릭터 보이스 차별화 — Character Voice Differentiation]
한 인물의 회피·동의·놀람·침묵 같은 감정 반응을 같은 어휘로 3회 이상 쓰면
AI가 만든 캐릭터 티가 난다. 어휘 빈곤은 "캐릭터의 결"을 무너뜨린다.

❌ 어휘 빈곤 (재원이 회피할 때마다 같은 표현):
S#9: "손님이시니까."
S#39: "손님이시니까."
S#59: "손님이시니까."
S#86: "손님이시니까."
→ 4회 같은 회피 어휘 = 회피 습관이 아니라 캐릭터 어휘 부재로 읽힌다.

✅ 회피의 변주 (같은 함수, 다른 어휘):
S#9: "손님이시니까."        ← 첫 등장. 거리 유지.
S#39: "그러게요."           ← 짧은 동의로 빠짐.
S#59: 침묵 + 다른 행동       ← 대답 자체를 회피.
S#86: "손님이시니까."        ← 마지막 등장. 첫 만남과 수미상관.
→ 회피의 4가지 변주가 캐릭터의 어휘 풀을 형성한다.

원칙 — 비트 집필 시 자가 점검:
1. 한 인물의 같은 감정 반응(회피·동의·놀람·침묵 등)을 같은 어휘로 3회 이상
   사용하지 않는다. 처음과 마지막에만 동일 어휘 = 수미상관 효과로 OK.
2. 인물별 회피 어휘 풀을 4가지 이상 마련한다:
   - 짧은 동의 ("그러게요" / "글쎄요")
   - 침묵 + 다른 행동
   - 농담으로 빠지기
   - 화제 전환
   - 질문으로 되받기
3. 인물별 시그니처 주저 패턴을 다르게 설계한다:
   - 데이터형 인물의 주저: 침묵 + "..." + 펜 멈춤
   - 감정형 인물의 주저: 다른 화제로 전환
   - 회피형 인물의 주저: 동작으로 회피 (커피 내리기 등)
4. 인물별 평균 문장 길이를 차별화한다 — 데이터형은 짧고 정확하게,
   감정형은 길고 비유적으로, 액션형은 짧고 단정적으로.

[A26. 중반 침식 차단 — Midpoint Erosion]
AI 시나리오의 가장 큰 문제: Beat 6~10(중반) 정체. 같은 공간에서 같은 패턴이
반복되면서 긴장 곡선이 평탄해진다. 이건 단순 룰셋으로 해결되지 않으므로
"질적 변화"를 강제해야 한다.

❌ 정체된 중반 (같은 공간 + 같은 패턴 반복):
Beat 6: 수진이 서점에 자료 들고 간다 → 재원 거부 → 흔들림
Beat 7: 수진이 서점에 자료 들고 간다 → 재원 거부 → 흔들림
Beat 8: 수진이 서점에 자료 들고 간다 → 재원 거부 → 흔들림
Beat 9: 수진이 서점에 자료 들고 간다 → 재원 거부 → 흔들림
→ 온도만 미세하게 올라갈 뿐 긴장 에스컬레이션 없음 = 중반 처짐.

✅ 변주된 중반 (거부 방식의 질적 에스컬레이션):
Beat 6: 수진이 자료 → 재원 무시 (1단계: 데이터 무시)
Beat 7: 수진이 자료 → 재원이 데이터로 반격 ("줄 서요. 여기.")
Beat 8: 수진이 의자에 앉음 → 재원이 공간 전복 ("거꾸로 집으셨는데요")
Beat 9: 수진이 침묵 → 재원이 정체 폭로 ("처음부터 알고 있었어요")
→ 거부의 4단계 에스컬레이션이 자체로 서사가 된다.

원칙 — 중반 비트(Beat 6~10) 집필 시 자가 점검:
1. **공간 변주 강제** — 메인 공간(예: 서점) 4회 이상 연속 등장 금지.
   2~3회 등장 후 반드시 다른 공간(외부 골목, 카페, 인물의 집)으로 전환.
2. **거부/장애물의 질적 변주** — 메인 갈등의 거부 방식이 비트마다 달라야 함:
   - 1단계: 무시 (반응 자체를 안 함)
   - 2단계: 반격 (상대의 무기로 받아침)
   - 3단계: 위치 전복 (공간/관계의 룰을 바꿈)
   - 4단계: 정체 폭로 (정보 우위 뒤집기)
   같은 단계 2회 이상 반복 시 비트가 정체된다.
3. **B스토리 능동 진입** — 중반에 B스토리(서브 캐릭터/서브 플롯)가
   최소 1회 메인 플롯을 흔드는 사건을 만든다. 단순 관찰자/공감자 금지.
4. **씬 길이 변주** — 중반 5개 비트 안에 단편 씬(<200자) 최소 1개 + 장편 씬(>500자) 최소 1개
   포함. 중편 씬만 연속되면 리듬이 단조롭다.
5. **중간점(Midpoint) 결정타** — Beat 8에서 반드시 "가짜 승리" 또는 "가짜 패배" 발생.
   중간점이 없으면 1막과 2막이 같은 톤으로 연결되어 정체된다.

[A27. POV 회전 강제 — POV Rotation Enforcer]
A18(단일 시점 고정 금지)이 "씬당 상대역 POV 1개"라는 약한 룰이라면,
A27은 작품 전체의 POV 균형을 강제한다.

❌ 단일 POV 편향 (수진 POV 80% / 재원 POV 20%):
대부분 씬이 "수진이 ..." 주어로 시작.
재원 단독 씬은 1~2개뿐.
재원의 내면이 보이지 않음 → 멜로 상대역으로서 매력 약함.

✅ 균형된 POV (수진 60% / 재원 30% / 기타 10%):
수진 단독 씬과 재원 단독 씬이 모두 존재.
재원이 수진을 관찰하는 씬과 수진이 재원을 관찰하는 씬이 균형.
관객이 양쪽 인물에 감정 투자 가능.

원칙 — 작품 전체 시점 균형:
1. **메인 인물 2명이 있는 작품(로맨스/멜로/버디)**:
   - 메인 인물 A 주어 비율 ≤ 70%
   - 메인 인물 B 주어 비율 ≥ 25%
   - 양쪽 단독 씬(혼자 등장하는 씬) 최소 2씬씩
2. **시선의 호혜성 강제**:
   A가 B를 관찰하는 씬 수 ≈ B가 A를 관찰하는 씬 수 (±30% 이내)
3. **B의 내면 진입 씬** — 서브 인물 단독 씬에서 그의 욕망/결핍이 한 번
   이상 행동으로 드러나야 한다. 단순 보조자/관찰자 금지.
4. **POV 변주는 정보 비대칭의 기회**:
   A 단독 씬에서 A만 아는 정보 1개,
   B 단독 씬에서 B만 아는 정보 1개를 심는다.
   이게 클라이맥스에서 정보 충돌로 작동한다.

━━━━━━━━━━━━━━━━━━━━━━━━━━━
ADVANCED CRAFT — 8점 진입을 위한 룰 (A28~A29, v3.7.0)
━━━━━━━━━━━━━━━━━━━━━━━━━━━

A25~A27이 \"안정적 7점\"의 룰이었다면,
A28~A29는 \"7점 천장을 뚫고 8점에 진입\"하는 룰이다.
Rewrite Engine의 7점대 정체 작품 분석에서 공통으로 발견된 2종 결함을 차단한다.

[A28. 행동 사이클 반복 차단 — Action Cycle Loop Block]
주인공의 \"공간 이동 시퀀스\" 자체가 3비트 이상 반복되면, 개별 비트는 잘
써졌더라도 관객의 긴장 곡선이 평탄해진다. A26은 \"같은 공간 4회 연속\"을
잡지만, A28은 \"같은 행동 시퀀스 패턴\"을 잡는다.

핵심 차이:
  A26 — 공간 카운트 (X공간 4회 등장 금지)
  A28 — 행동 시퀀스 패턴 (A→B→C 사이클 3회 반복 금지)

❌ 행동 사이클 반복 (호러 시나리오 사례):
Beat 6:  봉인실 진입 → 단서 발견 → 카페에서 조력자에게 확인 → 저택 복귀
Beat 7:  봉인실 진입 → 단서 발견 → 카페에서 조력자에게 확인 → 저택 복귀
Beat 8:  봉인실 진입 → 단서 발견 → 카페에서 조력자에게 확인 → 저택 복귀
→ 각 비트의 단서 내용은 다르지만, 동선이 동일하여 관객이 \"또 카페냐\"가 된다.
  중반 긴장이 평탄해지고 7점 천장에 갇힌다.

❌ 행동 사이클 반복 (로맨틱 코미디 사례):
Beat 6:  주인공 카페 방문 → 상대 거부 → 친구에게 하소연 → 다음날 재시도
Beat 7:  주인공 카페 방문 → 상대 거부 → 친구에게 하소연 → 다음날 재시도
Beat 8:  주인공 카페 방문 → 상대 거부 → 친구에게 하소연 → 다음날 재시도
→ 거부의 강도만 미세하게 올라가고 사이클 자체는 동일. 정체.

✅ 행동 사이클 변주 (사이클 자체를 다르게):
Beat 6:  봉인실 진입 → Siti 첫 조우 → 카페 조력자 확인 → 저택 복귀
         (사이클: 발견-외부확인-복귀)
Beat 7:  저택 정원에서 어머니 편지 발견 → 봉인실 진입 → 조력자 연락 두절
         (사이클: 외부발견-내부확인-고립화)
Beat 8:  봉인실에서 적대자 목격 → 도주 시도 → 담장 자물쇠 차단
         (사이클: 내부폭로-탈출-감금확인)
→ 같은 \"봉인실\" 공간이 등장해도 사이클의 기능이 달라진다.
  관객이 다음 비트의 패턴을 예측할 수 없다.

원칙 — 비트 집필 직전 자가 점검 (특히 Beat 6~12 중반·후반):

1. **3비트 사이클 비교 강제** — 현재 비트 집필 시 직전 2비트의
   행동 시퀀스를 비교한다. 다음 구조가 같으면 패턴 반복으로 판정:
   - 시작 공간 → 사건 발생 공간 → 외부 조력 공간 → 복귀 공간
   - 같은 사이클 = [공간 순서가 같음] AND [사건 기능이 같음(발견·확인·복귀)]

2. **사이클 변주 4가지 기법** — 다음 중 최소 1개 적용:
   - **공간 순서 역전**: 외부→내부 사이클을 내부→외부로 뒤집기
   - **사이클 압축**: 4단계 사이클을 2단계로 줄여 충격 가속
   - **사이클 차단**: 평소 가능했던 이동 경로 차단 (조력자 연락 두절, 자물쇠 등)
   - **사이클 침범**: 외부 인물이 내부 공간으로 들어와 사이클 깨기

3. **조력자 만남의 일회성 원칙** — 외부 조력자(전문가·친구·증언자)와의
   같은 공간(카페·연구실·바)에서의 만남은 작품 전체에서 최대 2회.
   3회째부터는 다음 중 하나로 변주:
   - 만남 공간 변경 (카페→공중전화→상대의 집)
   - 만남 형식 변경 (대면→비대면→문서로만)
   - 조력자 역할 전환 (정보 제공자→피해자→배신자)

4. **\"또 X냐\" 자가 점검** — 비트 완성 후 직전 2비트와 함께 읽어보고
   \"또 X(공간)냐\", \"또 Y(인물)냐\"라는 인상이 들면 사이클 반복이다.
   재집필 대상.

[A29. 물리적 대가 에스컬레이션 — Physical Cost Escalation]
주인공이 위험한 선택(금기 위반·진실 추적·적과의 대면)을 할 때마다,
그 대가가 \"주인공의 몸에 누적되는 흔적\"으로 가시화되지 않으면
관객은 위험을 체감하지 못한다. 호러·스릴러·액션·범죄 장르에서 8점 진입의 필수 조건.

핵심 원칙:
  - **대가는 타인이 아니라 주인공의 몸에 남아야 한다**
  - **대가는 사라지지 않고 누적되어야 한다**
  - **대가는 단계적으로 강도가 올라가야 한다**

❌ 대가의 비가시화 (호러):
Beat 6:  주인공이 봉인실 진입 → 손바닥에 붉은 자국 → 곧 사라짐
Beat 8:  주인공이 더 위험한 의례 수행 → 흔적 없음
Beat 10: 주인공이 적과 직접 대면 → 흔적 없음
→ 주인공이 멀쩡하니 관객은 위험을 체감하지 못함. \"왜 안 떠나는가\" 의문 발생.

❌ 대가의 타자화 (스릴러):
Beat 6:  주인공이 진실 추적 → 조력자 사망
Beat 8:  주인공이 더 깊이 추적 → 다른 조력자 사망
Beat 10: 주인공이 가장 깊이 추적 → 또 다른 조력자 사망
→ 주인공 자신의 몸에 대가가 없으면 \"주인공 무대가 부재\"로 7점 천장.

✅ 대가의 4단계 에스컬레이션 (호러 사례):
Beat 1~5  (1단계): 손바닥에 일시적 열기 자국 → 몇 분 후 사라짐
Beat 6~8  (2단계): 손등에 봉인 문양 흔적 → 며칠간 지속, 옷으로 가림
Beat 9~11 (3단계): 손끝 피부 아래로 먹선이 스며들어 영구화
Beat 12~  (4단계): 목소리에 진동, 호흡 곤란, 체온 저하 — 일상 행동까지 제약
→ 비트가 진행될수록 대가가 더 깊이, 더 넓게, 더 오래 남는다.
  관객이 \"이제 돌이킬 수 없다\"를 몸으로 안다.

✅ 대가의 4단계 에스컬레이션 (스릴러 사례):
Beat 1~5  (1단계): 잠 못 자는 밤, 손떨림 — 심리적 흔적
Beat 6~8  (2단계): 적의 1차 폭력 — 상처, 의료 처치 필요
Beat 9~11 (3단계): 2차 폭력 — 영구 흉터, 회복 불가
Beat 12~  (4단계): 신체 일부 영구 손상 (청력·시력·손가락 등)
→ 각 단계가 비가역적으로 누적.

원칙 — 비트 집필 시 자가 점검:

1. **대가의 위치 — 주인공 자신의 몸** — 다음은 대가가 아니다:
   ❌ 조력자의 부상·사망 (그건 \"타인의 대가\")
   ❌ 손바닥 자국이 곧 사라짐 (그건 \"불편함\")
   ❌ 문자 전송 실패 (그건 \"환경의 방해\")
   ✅ 주인공 몸에 남은 흔적 (사라지지 않는 것)
   ✅ 주인공 신체 기능의 일부 손실 (목소리·시력·손동작 등)
   ✅ 주인공 일상 행동의 제약 (잠·식사·외출의 변화)

2. **4단계 에스컬레이션 표 — 비트 집필 시 위치 확인**:
   | 비트 | 단계 | 대가 성격 |
   |------|------|----------|
   | 1~5  | 1단계 | 일시적·심리적 흔적 (불안, 손떨림, 짧은 자국) |
   | 6~8  | 2단계 | 가시적·일시적 신체 흔적 (멍, 상처, 며칠 지속) |
   | 9~11 | 3단계 | 영구적·국부적 흔적 (흉터, 영구 자국, 비가역) |
   | 12~  | 4단계 | 기능적·전신적 손상 (감각·운동·일상 행동 제약) |

3. **누적성 원칙 — 이전 대가는 사라지지 않는다**:
   - Beat 6의 자국이 Beat 8에서도 보여야 한다
   - Beat 8의 상처가 Beat 10에서 회복되어 있으면 \"리셋\"
   - 리셋된 대가는 대가가 아니다

4. **장르별 대가의 결**:
   - **호러**: 초자연적 흔적 (봉인 문양 전이, 차가워지는 체온, 목소리 변화)
   - **스릴러**: 폭력·추적의 흔적 (멍, 흉터, 청각·시각 손상)
   - **액션**: 전투의 흔적 (부상의 누적, 동작 제약)
   - **범죄**: 도덕적·신체적 흔적 (불면, 손떨림, 알코올 의존)
   - **드라마**: 정서적·신체적 흔적 (식이장애, 수면장애, 거울 회피)
   - **로맨스/멜로**: 감정의 신체화 (떨림, 식욕 변화, 목소리 흔들림 — 단, 1~2단계 위주)

5. **대가 없는 비트 = 재집필 대상** — 위험한 선택(금기 위반·진실 추적·적
   대면)이 있는 비트에서 주인공의 몸에 흔적이 남지 않으면 그 비트는
   \"가짜 위험\"이다. 관객이 위험을 체감하지 못한다. 재집필.

[A31. 첫 등장 괄호 정보 제한 — (나이대, 성별)만 허용]
캐릭터가 지문에 첫 등장할 때 이름 뒤 괄호에는 **나이대와 성별만** 적어라.
외양·복식·표정·체격·헤어·소품 묘사를 괄호에 넣지 마라.

이유 — 시나리오는 설계도이지 완성된 그림이 아니다. 인물이 어떻게 생겼는지,
무엇을 입었는지, 표정이 어떤지는 캐스팅·미술·의상·분장·연출 파트가 정한다.
작가가 괄호에 외양을 박아두면 그 파트들의 재량을 침범하고, 시나리오를 무겁게 만든다.
작가의 일은 \"누가\"(나이대·성별)까지다. \"어떻게 보이는가\"는 작가의 일이 아니다.

❌ 괄호에 외양·복식·표정을 넣는 습관:
   법무팀장(50대, 넥타이 단정)
   리셉션 직원(20대, 여, 단정한 정장)
   강도현(35, 구겨진 점퍼, 피곤한 눈)
   한석훈(40대, 날카로운 인상, 값비싼 슈트)
→ 넥타이·정장·점퍼·인상·슈트는 전부 의상팀·분장팀·캐스팅의 영역.
   작가가 괄호에 적으면 월권이고, 지문이 무거워진다.
...
✅ 나이대·성별만:
   ...
   강도현(35, 남)
   한석훈(40대, 남)
→ 작가가 정할 정보는 여기까지. 깔끔하고 프로다운 첫 등장 표기.

표기 형식:
  - 나이는 \"30대\" 또는 구체 숫자 \"35\" 둘 다 허용. 작품 톤에 맞게 일관되게.
  - 성별은 남/여로 표기. 맥락상 자명하면 생략 가능(예: 이름이 명백히 성별을 드러낼 때).
  - 괄호 안 항목은 최대 2개(나이대, 성별). 그 이상 넣지 마라.

예외 — 외양 디테일이 \"서사에 직접 기능\"할 때만 지문 본문(괄호가 아니라)에 적어라:
  - 그 디테일이 플롯의 단서일 때 (예: 한쪽 손의 화상 흉터가 정체를 밝히는 열쇠)
  - 그 디테일이 인물 관계·계급을 즉시 드러내는 장치일 때
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━
A34. 대사의 AI 과잉 말투 제거 — Subtext & Spoken Voice Mandate
━━━━━━━━━━━━━━━━━━━━━━━━━━━

핵심 진단: AI 대사는 두 가지로 티가 난다.
(1) 뜻을 남김없이 말로 옮긴다 — 겉말과 속뜻이 100% 일치한다.
(2) 표준 문어체 완성문으로 수렴한다 — 모든 인물이 매끈한 같은 말투를 쓴다.
둘 다 "설명"이지 "대사"가 아니다.

대전제 — 대사는 눈으로 읽는 문장이 아니라 배우가 소리 내는 말이다.
사람의 말은 부족하고, 흘리고, 생략하고, 딴 데로 튄다.
표준 문어체 완성문(주어·목적어 다 갖춘 매끄러운 서술문)으로 쓰지 마라.
모든 대사는 인물의 행동이다. 정보를 전하려는 말이 아니라 상대에게서
무언가를 얻어내려는(설득·압박·유혹·회피·공격·방어) 말이다.

집필 후 모든 대사를 네 검문으로 걸러라:

[검문 1 — 역전] 겉말과 속뜻이 일치하는가?
  일치하면 서브텍스트 없음 → 다시 써라.
  BAD: "나는 너에게 실망했고 우리 관계는 끝났어." (뜻을 다 말함 = 설명)
  GOOD: 뜻을 뒤집거나 비껴가는 말로. 다정한 어미에 위협을, 존대에 경멸을,
     무뚝뚝한 말에 걱정을 실어라. 형식과 내용이 어긋날 때 서브텍스트가 산다.
     예: 죽어가는 자에게 건네는 무심한 한마디가 실은 살라는 당부일 때.

[검문 2 — 감정 우회] 감정을 직접 설명하는가?
  "화가 나 / 슬퍼 / 너 때문에 허탈해" = AI 티.
  감정은 딴소리·행동·침묵으로 드러내라. 결정적 순간일수록 인물은 감정을
  정면으로 말하지 않고 엉뚱한 데로 튼다. (심문 끝에 끼니를 묻는 식)

[검문 3 — 말맛] 같은 뜻의 더 짧고 입에 붙는 말이 있는가?
  의미가 맞아도 소리가 밋밋하면 AI 대사다. 표준어 완성문보다 입에 붙는
  구어·축약·리듬을 골라라. "두 배로"보다 "더블로"가 사는 이유다.

[검문 4 — 완결] 병렬 대칭으로 뜻을 다 닫는가?
  "A도 안 했고 B도 안 했어" 식으로 정보를 대칭 완결하면 AI 티.
  사람은 뒷말을 흘리거나 상대가 끊는다. 말끝 흘림, 끊김, 되묻기,
  겹쳐 말하기를 허용하라.

[균형 안전장치 — 필독]
서브텍스트를 넣는다고 뜻이 안 통하면 안 된다. 관객이 맥락·표정·행동으로
속뜻을 찾을 단서는 반드시 남겨라. 우리는 상업 시나리오다.
단서 없는 서브텍스트는 난해한 아트하우스 대사다 → 그것도 실패다.

[말투 변형 — 강제 조항일 때만]
사투리·옛날말(고어)·외국어 같은 말투 변형은 인물 설정에 명시적 근거가
있을 때만 반영한다.
- 근거 있음(예: 인물 설정에 "강진 토박이 주모", "조선 사대부") -> 반영.
  * 사투리는 명사 치환이 아니라 어미·종결·리듬으로 산다. 명사만 사투리로
    바꾸고 문장 종결은 표준어인 짝짝이 대사(가짜 사투리)를 쓰지 마라.
  * 확실히 아는 어미·어휘만 쓴다. 불확실하면 표준 구어체 + 리듬으로 개성을 낸다.
- 근거 없음 -> 표준 구어체를 유지한다. 억지 사투리·억지 고어를 지어내지 마라.
  안 쓰느니만 못하다.
- 같은 인물이라도 상대·상황에 따라 말투는 달라질 수 있다(공식 자리 표준어,
  고향 사람과 사투리). 근거가 있을 때만 이 층위를 살린다.
- 최종 방언·시대어 고증은 작가의 몫이다. 엔진은 구어의 리듬과 개별화까지
  책임지고, 정확한 지역어 표기는 작가가 검수한다.

[적용 범위]
A34는 대사에만 적용한다. 지문은 A21~A23으로 관리된다.
검문 1·2·4는 장르 불문 항상 적용. 검문 3(말맛)은 순수 드라마·아트하우스
톤이 명시된 경우 작품 톤에 맞춰 완화 가능. 말투 변형은 강제 조항일 때만.

━━━━━━━━━━━━━━━━━━━━━━━━━━━
A35. 대사의 핑퐁 리듬 — Ping-Pong, No Monologue
━━━━━━━━━━━━━━━━━━━━━━━━━━━

핵심: 대사는 주고받는 것이다. 한 인물이 길게 혼자 말하면 대화가 아니라
연설이고, 관객은 지루해한다. 상대는 듣기만 하는 벽이 아니라 되받아치는
상대다.

[일방 연설 금지 — 장르 불문]
한 인물이 길게 혼자 말하는 비트를 쓰지 마라. 설득·고백·다툼·취조일수록
상대가 되받아친다. 형사가 범인에게 일방적으로 연설하지 않는다.
BAD: A가 자기 논리·감정을 길게 설명하고 B는 듣기만 한다.
GOOD: A가 말하면 B가 즉각 짧게 받아쳐 A의 말을 흔든다.

[받아치기 — 겹침이 아니라 즉각 반응]
상대의 마지막 말을 물고 들어가라. 각자 할 말만 하면 대화가 아니라
교대 독백이다. 단, 대사를 물리적으로 겹치지(오버랩) 마라 — 한국 관객은
대사 전달에 예민하다. 대사는 겹치지 않되 딜레이 없이 즉각 이어진다.

[긴 대사 예외 — 그 말하기가 사건일 때만]
긴 대사가 극적으로 필요한 순간은 있다(법정 최후변론, 유서, 사상의 토로).
단, 긴 대사는 "그 말하기 자체가 판을 뒤집는 행동"일 때만 허용한다
(A32·A33과 같은 기준). 일상·다툼·정보 전달 씬에서 긴 독백은 금지.

[적용 조건]
- 일방 연설 금지는 장르 불문 항상 적용.
- 핑퐁의 속도·강도는 장르로 조절한다. 로코·스릴러(취조)는 강하게,
...
# ★ v3.2.0 신규 — ROMCOM (코미디+로맨스 결합) 판별
def _is_romcom(genre: str) -> bool:
    """장르가 로맨틱 코미디인지 판별 (v3.2.0).
    1) '로맨틱 코미디'/'롬코'/'romcom' 명시
    2) 코미디 + 로맨스 동시 포함
    """
    g = genre.lower()
    ...
    if "romcom" in g or "rom-com" in g or "romantic comedy" in g:
        return True
    # 코미디 + 로맨스 동시 포함
    return _is_comedy(genre) and _is_romance(genre)


# ★ v3.2.1 신규 — 단독 코미디 판별 (ROMCOM/범죄코미디 등 결합 제외)
def _is_comedy_only(genre: str) -> bool:
    """단독 코미디 장르 판별 (v3.2.1).
    코미디이지만 로맨스/범죄/액션/호러 등 다른 부스터 대상 결합이 아닌 경우.
    예: '코미디', '코믹 드라마' (드라마는 부스터 대상이지만 우선순위 낮음)
    """
    if not _is_comedy(genre):
        return False
    # 다른 강한 부스터 대상과 결합되어 있으면 그쪽 우선
    if _is_romcom(genre):       return False  # ROMCOM 우선
    if _is_horror(genre):       return False  # 코믹 호러 → HORROR
    if _is_thriller(genre):     return False  # 범죄 코미디 → THRILLER
    if _is_action(genre):       return False  # 코믹 액션 → ACTION
    return True


# ★ v3.2.1 신규 — 미스터리·추리 판별
def _is_mystery(genre: str) -> bool:
    """미스터리/추리 장르 판별 (v3.2.1).
    스릴러와는 별개의 DNA — 단서·추리·퍼즐 중심.
    """
    g = genre.lower()
    ...
    if "추리" in g:
        return True
    if "탐정" in g or "detective" in g:
        return True
    if "후더닛" in g or "whodunit" in g:
        return True
    return False


# ★ v3.2.1 신규 — 사극·시대극 판별
def _is_period(genre: str) -> bool:
    """사극·시대극 장르 판별 (v3.2.1).
    Period Pack은 시대 디테일을 담당, 본 부스터는 비트 코어 DNA를 담당.
    """
    g = genre.lower()
    keywords = [
...
# ★ v3.2.1 신규 — 청춘·성장 영화 판별
def _is_coming_of_age(genre: str) -> bool:
    """청춘·성장 영화 판별 (v3.2.1).
    드라마와 별개의 DNA — 성장 단계의 캐릭터 변화 중심.
    """
    g = genre.lower()
    keywords = [
        "청춘", "성장", "coming-of-age", "coming of age",
        "학원물", "학원", "학교",  # 학원물(고등학교 등)
        "하이틴", "high school", "highschool",
    ]
    return any(kw in g for kw in keywords)


# ★ v3.2.1 신규 — 재난 영화 판별
def _is_disaster(genre: str) -> bool:
    """재난 영화 판별 (v3.2.1)."""
    g = genre.lower()
    keywords = [
        "재난", "disaster",
        "참사", "재해",
        "지진", "쓰나미", "화재", "전염병", "팬데믹", "pandemic",
        "엑시트", "터널", "감기",  # 한국 재난물 레퍼런스
    ]
    return any(kw in g for kw in keywords)
...
1. 신체 감각과 거리를 묘사하라 — 감정을 말하지 말고 몸의 반응으로.
   ❌ 드라마체: '그녀를 보고 마음이 설렜다.'
   ✅ 로맨스체: '손이 닿을 뻔했다. 그녀가 먼저 손을 뺀다. 손끝이 화끈거린다.'
...
□ 로맨스 상대와의 씬에서 신체 언어가 작동하는가?
  · 거리 (가까워짐/멀어짐/정지) 명시
  · 시선 교환의 길이 (스치는 시선/머무는 시선/오래 보는 시선)
  · 접촉의 의도 (의도적/우연/회피)
...
def build_write_beat_prompt(
    ...
    # ★ v3.8.0 신규 블록 — A30 미시 연출 반복 차단 / 작가주의 라이선스 자동 분기
    # 입력 필드를 합쳐 작가주의 여부를 감지(Creator v2.6.1과 동일 키워드 셋).
    #   상업영화(기본) → A30_DIRECTION_REPETITION_BLOCK (오브제·습관동작·초단위멈춤 반복 차단)
    #   작가주의       → A30_ARTHOUSE_LICENSE (시그니처 오브제·미시 연출 허용)
    # build_write_beat_prompt가 이미 받는 필드만 스캔 — 시그니처 무수정.
    arthouse_scan = (
        (genre or "")
        + "\n" + (logline or "")
        + "\n" + (world or "")
    ...
    direction_repetition_block = (
        A30_ARTHOUSE_LICENSE if _detect_arthouse_writer(arthouse_scan)
        else A30_DIRECTION_REPETITION_BLOCK
    )

    # ★ v3.2.0 신규 블록 — GENRE BOOSTER (장르별 비트 강제 룰)
...
14. 지문(Action Line):
   ...
   - ★ 같은 동선의 연속 동작은 반드시 한 문장으로 묶어라 (인과 어미 ~고/~며/~아 사용):
     ❌ "수현이 서점 문을 닫고 나온다. 골목 계단 중간에 앉는다."
     ✅ "수현이 서점 문을 닫고 나와 골목 계단 중간에 앉는다."
     → 같은 인물의 같은 방향 동선은 무조건 한 문장. 끊지 마라.
   - ★ 시간 정밀 표기 금지(A19): "0.3초", "0.5초", "1초", "3초" 등 숫자 시간
     절대 사용하지 않는다. 작가의 영역이 아니라 연출자의 영역이다.
     필요하면 "잠깐", "한참", "[긴 정적]" 같은 모호한 시간어 사용.
   - ★ 헐리우드 작법 5체크 통과 (A21~A24 참조):
     ① 카메라가 찍을 수 있는가 ② 인과로 이어지는가(~자/~며)
     ③ 한 문장 = 한 샷인가 ④ 현재형 능동인가(~한다/~다)
     ⑤ 비트 = 문단인가 (한 사건 = 한 문단)
   - ★ 7점 진입 체크 (A25~A27):
     ⑥ 캐릭터별 어휘 차별화 (회피 어휘 3회 이상 동일 사용 금지)
     ⑦ 중반 비트(B6~B10): 같은 공간 4회 연속 등장 금지
     ⑧ POV 균형 (메인 인물 2명 작품: 단독 씬 양쪽 2씬 이상)
   - ★ 8점 진입 체크 (A28~A29, v3.7.0):
     ⑨ 행동 사이클 반복 차단 (직전 2비트와 같은 공간·기능 시퀀스 금지)
     ⑩ 물리적 대가 에스컬레이션 (위험한 선택 시 주인공 몸에 누적 흔적)
   - ★ 미시 연출 반복 차단 (A30, v3.8.0, 상업영화 한정 — 작가주의 면제):
     ⑪ 서사와 무관한 오브제(볼펜·라이터 등)를 비트마다 반복 등장시키지 않았는가
     ⑫ 정형 습관 동작(주머니에 손·펜 뚜껑 여닫기·한숨·머리 쓸어넘김 등)을 돌려쓰지 않았는가
     ⑬ 초 단위 멈춤("5초간 멈춘다"·"0.5초 정지" 등)으로 연출 영역을 침범하지 않았는가
   - ★ 첫 등장 괄호 정보 제한 (A31, v3.8.2):
     ⑭ 첫 등장 괄호에 나이대·성별 외의 외양·복식·표정·소품을 넣지 않았는가
        (넥타이·정장·인상·헤어·체격 등은 캐스팅·미술·의상·연출의 영역)
...
18. AI ESCAPE — 집필 후 A1~A31 패턴 자가 점검:
   감정 설명 지문(A1) / 같은 말투(A2) / 본 것 반복(A3) / 무대 연출(A4) /
   편의적 정보(A5) / 침묵 부재(A6) / 대사 대칭(A7) / 처음부터 시작(A8) /
   같은 씬 해소(A9) / 총칭적 감각(A10) / 물리적 논리 비약(A11) /
   관찰자 없는 숫자(A12) / 원인 없는 결과(A13) / 캐릭터 재소개·풀네임 반복(A14) /
   동작 반복 루프(A15) / 같은 정보 반복(A16) /
   부정문 중독(A17) / 단일 시점 고정(A18, 로맨스 씬) /
   ★ 시간 정밀 표기 금지(A19, "0.3초/0.5초/3초" 등 절대 금지) /
   핵심 명사 과다 반복(A20) /
   ★ 헐리우드 작법: 인과의 사슬(A21) / 한 문장=한 샷(A22) /
   현재형 능동(A23) / 동사 없는 단편(A24) /
   ★ 7점 진입 룰: 캐릭터 보이스 차별화(A25) / 중반 침식 차단(A26) /
   POV 회전 강제(A27) /
   ★ 8점 진입 룰(v3.7.0): 행동 사이클 반복 차단(A28) /
   물리적 대가 에스컬레이션(A29)
   ★ 미시 연출 반복 차단(A30, v3.8.0, 상업영화):
     서사 무관 오브제 반복 / 정형 습관 동작 반복(주머니에 손·펜 뚜껑·한숨 등) /
...
★★★ [지문↔대사 빈 줄 강제 — 한국 시나리오 표준 포맷] ★★★
- 지문 직후에 대사가 오면 사이에 **빈 줄 1개** 필수.
- 대사 블록(연속 대사 포함) 직후에 지문이 오면 사이에 **빈 줄 1개** 필수.
- 같은 화자의 연속 대사(대사1 → 대사2)는 빈 줄 없이 연속.
- 다른 화자 대사로 바뀌어도 빈 줄 없이 연속 (대사 블록 = 한 덩어리).
...
""".strip()
    ...
    # ★ v3.8.0 — A30 작가주의/상업 분기 (재집필 시에도 동일 적용)
    # 상업영화면 미시 연출 반복 차단 룰을, 작가주의면 허용 라이선스를 RULES에 주입.
    _arthouse_scan_tr = (
        (genre or "")
        + "\n" + (logline or "")
        + "\n" + (world or "")
    ...
    a30_block_tr = (
        A30_ARTHOUSE_LICENSE if _detect_arthouse_writer(_arthouse_scan_tr)
        else A30_DIRECTION_REPETITION_BLOCK
    )
...
9. ★ 8점 진입 룰(v3.7.0) — 재집필 시 반드시 점검:
   - A28 행동 사이클 반복 차단: 직전/직후 비트와 같은 [공간 순서 + 사건 기능]
     시퀀스가 반복되면 사이클 변주 (순서 역전 / 압축 / 차단 / 침범 중 1개 적용).
     같은 외부 조력자(카페·연구실) 만남이 3회째라면 공간·형식·역할 중 하나 변경.
   - A29 물리적 대가 에스컬레이션: 위험한 선택이 있는 비트에서 주인공의 몸에
     누적 흔적이 남는가? 타인의 대가(조력자 부상)는 대가가 아니다.
